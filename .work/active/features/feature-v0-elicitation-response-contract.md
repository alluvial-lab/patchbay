...
## Review record (standard, pass 1)

**Reviewer:** `openai-codex/gpt-5.6-sol` fresh-context cross-model pass (standard weight).
**Verdict:** Request changes — 4 Blockers, 2 Important, 0 Nits.

### Adjudication (orchestrator)

All four Blockers verified against the repository — confirmed real, not phantom.

- **Blocker 1 — Terminal responses break idempotent retry semantics** (`core/src/acceptance/elicitation_response.rs:35`, `core/src/acceptance/pipeline.rs:80-104`): **ACCEPTED, material.** The validation block runs before `append_dedup`. A legitimate retry of an already-answered elicitation (same command_id + idempotency_key) hits `active.is_terminal == true` after catch-up and rejects with `validation_failed` before dedup returns the existing command. This violates the stated-normative `RetryAfterTerminalReturnsExisting` property and `docs/PROTOCOL.md:135` ("a duplicate submission... returns the existing command record and state"). A real correctness regression in the response case. Must fix: reconcile the idempotent retry (exact duplicate) before the terminal-rejection path, or retain winning-response identity to exempt the true retry. Add a terminal-response retry regression test.
- **Blocker 2 — Declared payload `content_type` not enforced** (`core/src/acceptance/elicitation_response.rs:103-109`): **ACCEPTED.** `decode_response_payload` ignores `PayloadEnvelope.content_type`; valid protobuf bytes declared as JSON/binary/unspecified are accepted. Unit 1 said response payloads declare `PAYLOAD_CONTENT_TYPE_PROTOBUF`. Fix: validate the discriminator before decoding; add wrong/unspecified content_type rejection cases. `schema_ref` may stay ignored per D8.
- **Blocker 3 — Committed `invalid_response_policy` semantics silently ignored** (`elicitations.proto:43,104-109`, `docs/PROTOCOL.md:308`): **ACCEPTED — genuine semantic gap.** The validator always maps malformed input to pre-acceptance `validation_failed`, but the contract can declare terminal-on-invalid policies (`TERMINAL_DECLINED`/`TERMINAL_SUPERSEDED`/`TERMINAL_CANCELLED`) which PROTOCOL:308 says must terminalize the Elicitation, not reject the Operation pre-acceptance. This makes part of the active `response_contract` non-authoritative — exactly the un-enforced-surface problem this feature exists to prevent. This is a semantic 50/50 the design's D1 did not fully resolve (D1 framed validation as "reject malformed with validation_failed," sidestepping the policy). Resolution required: either (a) implement terminal-on-invalid behavior and test it, or (b) explicitly reclassify terminal-on-invalid policies as reserved for v0.1.0 (the Elicitation stays `pending` on invalid input; terminal-on-invalid is a future promotion) and align proto comments + PROTOCOL.md to say so. Option (b) is the smaller, non-foreclosing choice and keeps v0.1.0 single-behavior; the implementer should pick (b) unless (a) is clearly cleaner — and either way must make the decision explicit, not silent.
- **Blocker 4 — Foundation protocol contradicts the reserved multi-answer seam + D8 omitted** (`docs/PROTOCOL.md:312,321,331`): **ACCEPTED — foundation-doc drift.** PROTOCOL:331 still says `question` = "Answer one or more questions, possibly with options and freeform text," contradicting D2/D3 (single-answer; multi-answer is the reserved "multi-answer accumulation" seam at PROTOCOL:312). D8's promised statement (typed committed bodies are authoritative; `schema_ref` ignored for committed kinds) was not added. Rolling-foundation: roll PROTOCOL.md forward to settled single-answer + typed-body semantics. This is the extension-pressure-checklist obligation.
- **Important 1 — Vectors not executable through the pipeline** (acceptance vectors omit `sender`/`target_scope`/`idempotency_key`; rejection vectors use noncanonical `"validation_failed"` not `FAILURE_CODE_VALIDATION_FAILED`, no `SubmissionResult` shape): **ACCEPTED.** `check-vectors` is envelope-only (doesn't execute the pipeline), so these pass today, but they're not truthful submission contracts. Fix: complete the `response_operation` with the structurally-required fields and use canonical enum-string expectations, OR explicitly mark them pure-validator vectors. Prefer completing them so they're real.
- **Important 2 — Fold-lag test doesn't exercise `catch_up`** (`server/src/state.rs:309-356`): **ACCEPTED, not blocking.** Test calls `lookup.observe(&event)` directly, not storage-append + `ProjectionState::catch_up`. The service ordering is sound (verified), but the test doesn't prove cursor folding / gate coordination. Add a storage-backed catch-up test; note the shared-gate requirement for the future Elicitation-opening producer.
- **Rejected proposals:** multi-select/multi-question fields now (correctly reserved); block solely on vector non-executability (Rust validator tests are the executable check today); typed approval body this feature (D5 scope correct); Rust/TS generator drift (pre-existing, unrelated).
