...
1. **Stale-issuer race** — fixed by re-verifying the compound issuer after the shared `CoreDecisionGate` catches projections up, before submit, subscription authorization, snapshot/diagnostics decisions, and every session/principal revocation or enrollment mutation (`server/src/service.rs:252-257`, `server/src/service.rs:406-417`, `server/src/service.rs:740-803`, `server/src/service.rs:859-870`). The deterministic regression `stale_issuer_is_rejected_when_revocation_commits_before_acceptance` (`server/tests/grpc_smoke.rs:521-590`) holds the gate, commits a principal fence while the request waits, and confirms gRPC `UNAUTHENTICATED` with no acceptance.
2. **Audit completeness + attribution** — fixed by durable `LoginFailed` records for transport-principal authentication failures, `AuthorizationFailed` records for valid denied targets, and a durable audit on idempotent session/principal/endpoint/device (and grant) repeats. Revocation audits now keep verified revoker actor/endpoint/device attribution and encode every target in `target_scope` (`core/src/authority/operator.rs:480-546`, `core/src/authority/operator.rs:604-647`, `core/src/storage/audited.rs:197-231`, `server/src/service.rs:1305-1355`). Query assertions cover denied target, repeated principal revocation, third-endpoint endpoint attribution/target scope, and authentication failure (`server/tests/grpc_smoke.rs:379-517`).
3. **`revoked_session_count`** — fixed by returning the count from the single state-ingestion mutation site instead of invoking the same revocation a second time in handlers (`server/src/state.rs:473-514`, `server/src/service.rs:765-778`, `server/src/service.rs:831-845`, `server/src/service.rs:919-932`). gRPC assertions prove nonzero first-call counts and zero idempotent-repeat counts for principal/endpoint/device, plus nonzero revoke-all count (`server/tests/grpc_smoke.rs:242-378`).

Verification: `cargo build --workspace --all-targets`; `cargo test --workspace`; `cargo clippy --workspace --all-targets -- -D warnings`; `cd contracts/ts && npm run build && npm run check:vectors && npm run check:models && npm run check:drift`; `cd web-server && npm test`; `cd cli && npm test`; `cd web-cockpit && npm test`; `cd pi-adapter && npm test`; `cd e2e && npm test` all pass.
