...
- **Equality, not ordering, is the compatibility rule.** A snapshot is compatible only when its embedded authority domain and core generation exactly equal the current durable values and its embedded snapshot LSN exactly equals the storage row's `(authority_domain_id, LSN)` anchor. Snapshot freshness remains a separate LSN comparison. Missing/zero generation, wrong domain, wrong generation, undecodable payload, or an LSN mismatch makes the derived checkpoint unusable; the log remains authoritative and the caller repairs from current materialization or full replay.
...
- **Do not pre-decide the checkpoint namespace.** This feature retains the current session-only durable snapshot slot and on-demand resource materialization. `recovery-checkpoint-writer` still owns the later choice among a typed composite checkpoint or per-projection namespaces; either can consume the same durable epoch and anchor rule.
...
- The decoder checks, without normalization: stored `EventId` domain/LSN are present and match the expected domain; the Protobuf payload decodes as `SessionSnapshot`; embedded domain and nonzero generation are present and exactly match current values; embedded `snapshot_lsn` exactly equals the stored row LSN. It does not decide freshness or inspect session content.
...
- Keep validation in a small module reusable by `recovery-checkpoint-writer`; do not add a generic snapshot framework or change the current storage namespace.
...
- Reuse the wire-present `Generation` fields, existing authority-domain metadata shape, snapshot table, `StoredSnapshot`, server RNG infrastructure, and log replay fallback. Add no wire field, versioned payload, dual reader, second log, or parallel authority source.
- Preserve the current session-only checkpoint namespace; the downstream writer may replace it with a typed design in place when its projection scope is decided.
...
- **Derived legacy checkpoints have no generation.** Rejecting them causes a one-time full replay but cannot lose authoritative events. The fallback is intentionally replay, not a compatibility shim.
...
- **Committed v0.1.0:** one core-assigned, storage-persisted, nonzero continuity epoch per authority domain; stable across ordinary process restarts; exact domain/epoch/LSN snapshot compatibility; invalid derived checkpoints fall back to the durable log/current materialization.
