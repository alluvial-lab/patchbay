...
| `token-commune-unsupported-operation-terminalization` | `TokenCommuneAdapterFailureSafe` | Unexpected accepted Operations survive both a retryable terminal-report failure and a hard adapter replacement after acknowledgement: redelivery/reconnect finishes unsupported terminalization before later work, and core history ends in exactly one durable `DELIVERED` then one `REJECTED/UNSUPPORTED_COMMAND`. | From raw lifecycle events require one durable `DELIVERED` before one terminal `REJECTED`, no `COMPLETED` or `FAILED`, exact failure code, no duplicate durable acknowledgement, and no accepted/delivered command left non-terminal after either recovery path. | Clear pending state before successful terminalization; filter delivered-but-nonterminal work from replacement redelivery; advance to later delivery first; emit `COMPLETED`; use `ADAPTER_UNAVAILABLE`; append duplicate delivery/terminal transitions. |
| `token-commune-cockpit-presentation-honesty` | `TokenCommuneCockpitPresentationHonesty` | The exact adapter projection fixture renders carried fingerprint/share/draw/reset summaries, bounded pool/gap events, independent wrapper/reading ages, both missing-reading states, current, stale, unknown/invalid, cross-provider-model, forbidden-alias, and hostile renderer/contributor fields through the local operator-domain and option-7 panel. | Literal summary/DOM expectations require source-age visibility, unavailable missing readings, event projection, stale dominance, unknown anchoring, foreign-model omission, exact provider-local runnable evidence, no contributor/member/subkey or `gpt-5.6`, verdict provenance text, and no hostile HTML/script/renderer execution. | Ignore wrapper freshness; drop unknown rows; provider-only joins; accept or display cross-provider models/`gpt-5.6`; drop carried capabilities/source age/events; label missing readings current; expose contributor/member/subkey; trust adapter verdict; accept `rendererUrl`/HTML/script or use dynamic import/`innerHTML`. |
...
- Exercise two loss points after the real core has durably acknowledged
  delivery: one retryable `rejectUnsupported` failure in the same process, and one
...
- [ ] Unsupported work reaches exactly one delivered and one rejected/unsupported
...
5. **Failure-safe delivery:** durably accept two unsupported Operations; for one,
   ...
   durable delivered then one rejected/unsupported terminal outcome before later
   delivery.
...
## Deep-lane pass-2 remediation (2026-08-08)

- Kept `stage: review`; the reviewer/autopilot owns the next transition.
- TypeScript mutant graphs now live beneath the owning package, including the token adapter's schema assets, so package dependencies and package type resolve normally. Each witness records module-load completion before executing the mutant oracle. Setup, linkage/load, and cleanup failures are typed harness errors and cannot be reported as kills.
- Re-ran every retained TypeScript witness through baseline production and the same post-load oracle. Four redundant downstream redaction labels and the surviving clear-before-terminal-ack scenario were removed rather than credited; the no-anchor mutant was moved to the production branch the promoted rollover scenario actually executes.
- Rust degradation and source witnesses now use an explicitly feature-gated compiled service fault patch. Baseline and mutant requests cross `AdapterControlService`, and the independent oracle reads `ProjectionState::materialize_resource_snapshot`; no mutation witness calls `ResourceRegistry::get`, edits test actions, or bypasses ingress with direct storage append. Manufactured carry-forward, polling-liveness, reconnect-promotion, and payload-source storage-bypass entries were removed.
- The credential oracle scans every declared hostile value, including the literal credential-file path, across every sink and raw forwarded diagnostic payload bytes. The new path-only production mutant leaves key redaction intact, removes only absolute-path redaction from the compiled diagnostic sink, leaks the path, and fails the same oracle.
