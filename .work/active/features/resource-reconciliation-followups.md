...
- **Synchronize before normalizing a report.** `ingest_resource_report` incrementally folds the storage tail through the registry's applied cursor before deriving `from_revision_lsn` or generation effects. The existing composition-root `CoreDecisionGate` remains the serialization boundary between that catch-up and append. A committed fold failure still triggers authoritative rebuild; generation monotonicity is never weakened to recover.
...
- `catch_up_from_log` reads after `registry.applied_lsn()` and feeds every returned durable kind in order. `ingest_resource_report` calls it before `normalize_report`. Do not add a second storage head or snapshot cursor. The caller still owns serialization; production already uses `CoreDecisionGate`.
...
- **A caller can lie about coverage by feeding only resource events.** The cursor advances on every known durable event, report ingestion catches up from storage before normalization, and full replay requires gap-free order. Production's decision gate closes the catch-up/append race. Fallback: rebuild the resource projection from LSN 0; never infer coverage from record revisions.
...
- **Concurrent writers outside `CoreDecisionGate` could create a gap after catch-up.** Such a path violates the current single-core composition contract. Treat the fold error as corruption, rebuild, and surface the failure rather than retrying under a weakened generation rule.
