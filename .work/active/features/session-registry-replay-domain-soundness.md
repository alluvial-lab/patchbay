...
## Review notes — deep-lane Phase 2 security-lockdown fix

- Accepted blocker: the old lockdown test replayed an entry and exit consecutively, allowing omitted lockdown ledgering to transiently apply both faulty folds and finish equal; same-LSN lockdown conflict and failed-validation ledger poisoning also lacked dedicated witnesses. All three reviewer mutants passed the pre-fix 16-test registry suite.
- The integration child now checks old-entry and old-exit redelivery before any compensating event, conflicting same-LSN lockdown envelope rejection with exact non-mutation, and malformed-then-corrected same-LSN behavior. The omit-ledger, accept-any-same-LSN-lockdown, and pre-validation-LSN-claim mutants each fail their focused witness with exit 101 and were restored.
- Registry 18/18, the remaining focused feature suites, the named server gate regression, workspace tests/Clippy, model/vector checks, scoped formatting, and diff hygiene are green. Production remains unchanged; assurance remains implementation-checked only; both this feature and its verification child remain at `stage: review` for the caller's continuing Phase-2 convergence lane.

## Review notes — deep-lane Phase 2 lockdown evidence follow-up

- Accepted two further evidence blockers: exact-envelope mutation witnesses had not started from an applied lockdown envelope, and transition-specific lockdown validation lacked a corrected-same-LSN table proving rejected candidates never preclaim replay identity.
- The integration child now uses test-owned weak predicates over kind-only/same-bytes and valid unknown-field lockdown re-encodings; actual payload-only and decoded-semantic production mutants fail. Its three-row semantic table covers inner-domain conflict, affected-count mismatch, and exit while inactive; actual inner-domain-removal and late-preclaim mutants fail. All four temporary production edits were restored.
- Registry 20/20, all focused feature suites, the named server gate regression, workspace tests/Clippy, model/vector checks, scoped formatting, diff hygiene, and production zero-diff are green. Assurance remains implementation-checked only, with parent and child intentionally retained at `stage: review` for caller-owned adversarial convergence.

## Review notes — deep-lane Phase 2 tail 4 outer-domain ordering

- Accepted blocker: prior cross-domain cases used unapplied LSNs, so they did not prove an applied exact payload could not bypass authority-domain validation through replay classification.
- The integration child now covers applied `SessionState` and `SecurityLockdown` envelopes independently. Each witness preserves the LSN and complete `StoredEventPayload`, changes only the outer `EventId.authority_domain_id`, requires `AuthorityDomainMismatch`, and asserts exact registry non-mutation.
- Moving production domain validation below replay classification made both focused witnesses fail (exit 101); the mutant was restored. Registry 22/22, all focused feature suites, the named server gate regression, workspace tests/Clippy, model/vector checks, scoped formatting, diff hygiene, and production zero-diff are green. Assurance remains implementation-checked only, with parent and child retained at `stage: review` for caller-owned thorough convergence.

## Review notes — deep-lane Phase 2 tail 5 new-LSN semantic guards

- Accepted the caller's exact four-case evidence tail; no production defect was found. The caller explicitly selected highest `openai-codex/gpt-5.6-sol`, `thorough`, and no nested agents.
- A direct-registry table now rejects at a new LSN: a tombstoned-generation delta aimed through the live slot, a generation bump whose `from_generation` is not live, and connectivity/activity deltas whose declared prior state is false. Every row proves exact registry/ledger non-mutation, then applies a corrected envelope at the identical `EventId` and proves it changes state.
- Four actual guard-removal mutants were killed by the named table (each exit 101): the live-record generation equality check, generation-bump predecessor equality check, connectivity prior-state equality check, and activity prior-state equality check. Every production edit was restored; `core/src/session/registry.rs` has zero diff.
- Registry 23/23, all remaining focused suites, the named server gate regression, full workspace tests/Clippy, model/vector checks, scoped formatting, and diff hygiene are green. No model/vector/wire/foundation/backlog/unrelated change or temporary mutation residue remains; evidence is implementation-checked only.
- Pass count/cap: Phase 1 completeness converged in 1/5; Phase 2 adversarial is now 4/5 (five finding/fix passes overall). This is the final bounded evidence fix. Parent and verification child remain at `stage: review` for one caller-owned final fresh-context recheck, Phase 2 pass 5/5.

## Review notes — feature-review exact four-tail: diagnostics lockdown projection

- Accepted a production blocker: `DiagnosticsProjection::observe` forwarded only `SESSION_STATE` to its embedded `SessionRegistry` and treated `SECURITY_LOCKDOWN` as a diagnostics no-op. Adapter diagnostics therefore continued counting a registered live session as live during lockdown, and a cold replay later rejected the valid post-exit `stale -> live` report because its private session view had never been clamped stale.
- The fix routes every known non-`UNSPECIFIED` durable event through the embedded domain-bound `SessionRegistry` after common framing/kind validation and before diagnostics-specific dispatch. Session/security kinds then need no second diagnostics fold. This preserves the shared gap-free prefix validator as the complete-log authority while retaining exact owned-envelope redelivery and detecting a concrete sibling-kind substitution at an already applied session identity.
- Regression evidence covers both incremental application and fresh cold reconstruction for adapter registration + live session registration -> lockdown enter -> lockdown exit -> report-derived `stale -> live`. It checks live/stale/offline/failed adapter counts at every prefix, exact old owned-event redelivery after the final state, and exact projection non-mutation on a same-identity concrete sibling envelope. All three focused witnesses failed before the production fix (exit 101) and pass after it.
- Updated `.agents/skills/patterns/durable-log-projections.md` so the session-fold example constructs a fallible authority-domain-bound registry with `SessionRegistry::new(authority_domain_id.clone())?` rather than the removed domainless constructor.
- Verification: diagnostics projection 9/9; session registry 23/23; ingest 17/17; replay/resolver 9/9; session properties 9/9; acceptance 25/25; replay-integrity 7/7; named server `CoreDecisionGate` race regression passed; full workspace tests and workspace Clippy with warnings denied passed; model checks passed (53 modeled properties); vector checks passed (53 vectors, 16 promoted, 21 implementation checks, 37 mutation witnesses); scoped rustfmt and `git diff --check` passed.
- Caller boundary/capability: exact four-tail, highest `openai-codex/gpt-5.6-sol`, explicit `thorough`, direct-read only, no nested agents. No model, vector, wire, foundation, backlog, child-item, or unrelated change was made. The verification child remains `done`; this feature intentionally remains at `stage: review` for the caller-owned thorough recheck rather than self-closing.
