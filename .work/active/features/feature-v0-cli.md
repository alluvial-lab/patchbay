...
## Blocker (2026-07-20) — CLI auth posture (semantic 50/50, operator decision required)

The CLI is a first-class control surface that speaks gRPC directly to the core (SECURITY:143). But the protocol seam settled the **compound-issuer** wire shape around the *web-server* as the verified transport principal that vouches for the operator (it forwards `x-patchbay-operator-id` + `x-patchbay-operator-session-id` after verifying the operator's login cookie/CSRF). The CLI bypasses the web-server. So the question is: **how does the CLI establish the operator identity the core requires?**

Two defensible options produce materially different security/surface behavior:

1. **CLI runs its own operator-session bootstrap (full transport principal):** `setup`/`login` create an operator session locally (the CLI verifies the operator password against the same `scrypt$` record, OR consumes the one-time setup secret), then the CLI forwards `x-patchbay-operator-id` + `x-patchbay-operator-session-id` exactly like the web-server. The CLI is a full transport principal with its own endpoint/device record + generation, audited as such. This matches the compound-issuer model end-to-end (the core independently verifies the CLI principal + operator identity), and satisfies SECURITY:81 ("A CLI endpoint enrolls only through local setup credentials or an existing authenticated operator session"). It requires a CLI-local credential store (0600, file-locked) holding the session id + operator actor id + endpoint generation.
2. **CLI holds the core-secret + a configured operator actor id (admin/dev tool, no per-session login):** the CLI is configured with `PATCHBAY_CORE_SECRET` + `PATCHBAY_OPERATOR_ID` and forwards a fixed operator identity (no per-session login, no `OperatorSessionId`). Simpler, but it bypasses operator-session verification and the one-time setup-secret lifecycle. This weakens the audit trail (every CLI action is "the configured operator, no session") and does not satisfy SECURITY:81's "existing authenticated operator session" path — it would be a new, weaker v0.1.0 posture not currently in the foundation docs.

**This is a semantic 50/50, not a mechanical choice.** It affects the security model (does the CLI participate in the compound-issuer verification, or bypass it?), the audit trail (per-session CLI actions vs. a fixed identity), and the authority boundary (is the CLI a full transport principal, or an admin shortcut?). The foundation docs commit the CLI as a transport principal (SECURITY:143) and require CLI enrollment via setup/session (SECURITY:81) — which points toward option 1 — but they do not pin the exact v0.1.0 wire shape for the CLI, and option 2 is a defensible simpler v0.1.0 dev/admin posture. The decision is not mechanical; two reasonable implementers would pick different options and produce materially different security behavior.

Per the harness rule (`.agents/rules/`: a semantic 50/50 affecting a safety claim / authority boundary must surface as a blocker, not be resolved with judgment), this blocks advancement. The feature stays at `stage: drafting` until the operator decides. Resolving it is a design act, not implementation — option 1 may also surface a follow-on: whether the CLI's operator-session bootstrap should reuse the web-server's `SessionStore` code or carry its own (a mechanical choice once option 1 is chosen).
