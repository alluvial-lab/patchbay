...
- **Real-path evidence without routing expansion:** the strengthened E2E exercises authenticated adapter delivery, delivery acknowledgement, successful result, session report, background completion, SQLite restart, and a subsequent real control `Submit` authorized by the replayed descendant grant. It explicitly demonstrates that ordinary adapter/fleet spawn `Submit` still returns `target_not_found`; the accepted adapter-scoped handoff is seeded rather than silently inventing fleet routing.
