...
All eight child stories implemented and advanced to `stage: review` via `implement-orchestrator` (sequential wave, width 1 — the `depends_on` chain is strictly linear because every unit touches `check-vectors.mjs` and the generated VERIFICATION.md tables):
...
| 8 | `story-verification-correction-mutation-fragility-demotion` | `4702b9a` | review |

Units 1–6 were the original design; Unit 7 was added by the round-2 deep review (4 promoted properties with trace-fidelity/mutation-fragility defects the original design missed); Unit 8 was added by the round-4 deep review (9 leftover demoted formulas Units 1-2 had left behind, plus 9 surviving promoted properties confirmed mutation-fragile after the host retracted its round-3 dispute).

**Net verification-claim result:** 24 properties demoted from promoted (checked-model) to draft (stated-normative): 5 from `command_lifecycle.qnt`, 4 from `session_generation.qnt`/`elicitation_lifecycle.qnt`, 1 Alloy (`ActorIdsUnique`), 5 from `authority.qnt` (`SpawnCreatesDescendantGrant` + 4 trace-fidelity), 3 from `subscription_authority.qnt`, 6 from `elicitation_lifecycle.qnt`, 1 from `reply_correlation.qnt` (`TypedCorrelation`). 24 misleading/defective `val`/`temporal` formulas removed entirely (not stubbed to `true`) so `quint verify --invariant <name>` fails honestly rather than passing vacuously or on a defective formula. 7 retained promoted properties' `semantics:` text narrowed to match their formulas. `GenerationMonotonic` prose corrected (strict-supersession is action-enforced, not checked). 4 superseded toy artifacts relocated out of `specs/seed/` to skill example directories. Stale model header comments fixed across all affected models. Stale prose reconciled across PROTOCOL.md, VERIFICATION.md, ADAPTER-PI.md, SPEC.md, GLOSSARY.md.

**Final property tier counts:** 8 promoted / 39 stated-normative (36 modeled-draft + 3 reserved-unmodeled) = 47 total. Down from 32 promoted / 15 stated-normative pre-correction. The 8 survivors were independently mutation-tested in round 5 and confirmed to catch their claim-breaking mutations: `TerminalFinality`, `BoundaryDedup`, `NoAcceptedToCompleted` (command_lifecycle); `GenerationMonotonic` (session_generation); `CsrfRejectsMissingProof`, `CsrfRejectsUnauthenticated`, `RevokedSessionCannotCommand`, `browser_local_state_not_authority` (csrf_browser — fixed to inspect attempted evidence).
...
**Verification status:** `quint parse` exits 0 for all affected model files; `check-vectors.mjs` exits 0; `check-models.mjs` exits 0 (tables current). Generated VERIFICATION.md tables reflect the demotions. The 24 demoted properties each have a `demotion_reason` naming the exact defect; the 8 survivors were independently mutation-tested in round 5 and confirmed to catch their claim-breaking mutations.

**Deferred to v1 formal gate (`epic-public-product-contract-executable-release-assurance`):** real formulas for the 24 demoted properties (crash/restart durability, competing pre-append candidates, retry-input identity, returned-record identity, per-session identity tuple, label-override routing, stale-event audit, elicitation-timeout grant boundary, descendant-grant allowed-kind set with action-created state, snapshot/recovery failure boundaries, general authority, independent-actor/endpoint/scope verification, genuine non-cascade formula with pre/post state, Elicitation lifecycle finality/correlation/invalid-rejection/stale-target/withdrawal with attempted-evidence state, subscription audit/cursor-replay with attempted-evidence state, typed correlation with attempted-evidence state). Also deferred: the attempted-evidence model re-architecture (introducing immutable raw submitted-value state across the elicitation/subscription/reply-correlation model families) — see `idea-csrf-trace-fidelity`.
...
## Review round 5 (2026-07-11)

**Verdict**: Approve with comments (convergence reached on the core question)

**Lane**: deep, adversarial convergence pass, `openai-codex/gpt-5.6-sol` (xhigh), fresh-context.

**The convergence signal (high-value finding):** the reviewer independently mutation-tested all 8 surviving promoted properties and confirmed each CATCHES its claim-breaking mutation. The survivor set is genuinely sound: the 4 CSRF properties catch coordinated-lie mutations (the attempted-evidence architecture works); `TerminalFinality`/`BoundaryDedup`/`NoAcceptedToCompleted`/`GenerationMonotonic` catch transition/structural violations. No further demotions are warranted. The promoted survivor set has converged.

**Blockers** (2, both prose/operational drift Unit 8 left behind — fixed inline in this review stride):
1. `docs/VERIFICATION.md:548` and `docs/GLOSSARY.md:63` still described demoted Elicitation/subscription properties as checked-model. Fixed: both now state the properties are stated-normative with no executable formula.
