...
- Accepted:
  ...
  - Narrow the `@promotion` semantics text of five retained promoted properties whose formulas are genuine but whose descriptions overclaim: `NoAcceptedToCompleted` (metadata says "must pass through `delivered`" but formula permits `delivered` OR `running`), `FleetAuthorityForSpawn` and `SubscriptionGrantChecked` (claim "authenticated actor" but no authentication evidence modeled), `ElicitationResponderAuthority` (claims endpoint authentication but the model only checks the modeled endpoint-to-actor binding), and `browser_local_state_not_authority` (claims grant-check protection but no grant state).
...
Five retained promoted properties have formulas that are genuine (mutation-survivable, independent oracle) but whose `@promotion` semantics text overclaims what the formula establishes. Narrow the semantics text to match the formula, not demote the property.
