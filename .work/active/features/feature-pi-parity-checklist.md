...
4. **Required Pi adapter capabilities for v0** — the core checklist. Two distinct concerns, kept in separate columns so per-Operation delivery mapping is not conflated with manifest-field declarations (review finding I1): (a) the Pi wire action(s) that satisfy each committed `OperationKind`, and (b) the capability-manifest declarations the Pi adapter makes, using **actual manifest fields** from `docs/PROTOCOL.md` Adapter capabilities (`supported_operation_kinds` includes/excludes; `streaming`; `snapshot` tier; `cancellation`; `session_replacement`; `idempotency_strength`; `attachment_method`; `known_failure_modes`). Per-Operation delivery notes (e.g. `unsupported_command` is a *delivery outcome*, not a manifest field) belong in the mapping column, not the manifest column. Columns: `OperationKind` | Pi wire action(s) | manifest declaration (actual fields) | v0 disposition. Rows derived from the per-action Pi evidence in `feature-operator-presence-and-action-inventory`:
   - `attach` <- `session_sync` | `supported_operation_kinds` includes `attach`; `streaming=true`; `snapshot=partial`; `cancellation=true`; `session_replacement=true` | committed. (Note: `pair_request` is transport/pairing, NOT an `attach` wire action — see section 7.)
   - `instruct` <- `user_message` | `supported_operation_kinds` includes `instruct` | committed
   - `cancel`/`interrupt` <- `cancel` | `cancellation=true` (interrupt aliased to `cancel` or unsupported-by-adapter at delivery) | committed
   - `approval-response` <- `approve_tool` (approval Elicitation opened via the `tool_call` hook) | `supported_operation_kinds` includes `approval-response` | committed
   - `query` <- `session_sync`/`list_models`/`ping` | `supported_operation_kinds` includes `query` | committed
   - `reconfigure` <- `model_set`/`thinking_set` | `supported_operation_kinds` includes `reconfigure` | committed
   - `session-management` <- `session_new`/`session_compact` | `session_replacement=true` (session_new bumps `session_generation`; session_compact does not) | committed
   ...
   - Reserved `agent-send`/`adapter-utility-exec` <- n/a | excluded; submission rejects with `validation_failed` in v0 | reserved
   Include the manifest snapshot-tier declaration: Pi declares `snapshot = partial`.
...
6. **Commands as adapter-declared capabilities, not core states** — explicit restatement of the capability-not-authority and capability-not-delivery-gate rules from `docs/PROTOCOL.md`. Map `session_new`/`session_compact` as `session-management` Operations whose adapter-side effect (generation bump for `session_new`; in-place summarize for `session_compact`) is adapter-reported, not a core protocol state. Call out the `session_new` != `spawn` distinction: `session_new` resets the conversation on the **same Patchbay runtime/daemon slot** (`runtime_session_id` stable) and does not provision a new runtime. (Phrase precisely per review nit N1: in daemon mode remote_pi may intentionally restart the Pi RPC child without `--continue` to obtain the fresh session — that is the `EXIT_DAEMON_FRESH_SESSION` path at `index.ts:2216` — but that process restart is the mechanism by which the same daemon slot realizes a session replacement, not a `spawn` of a new Patchbay target.)
...
- confirm every `OperationKind` in the section 4 table matches the registry in `docs/PROTOCOL.md` (and the generated enum in `contracts/rust/src/gen/patchbay/patchbay.rs` — note the generated Rust uses `AdapterCapability` for the manifest and carries identity fields on `Session`/`TargetScope`, not structs literally named `CapabilityManifest`/`SessionIdentity`; check parity against the actual generated names, review nit N2);
- confirm the identity tuple in section 3 matches `docs/PROTOCOL.md` Sessions and the `session_generation.qnt` variables;
- confirm `session_new` is classified as a generation-bumping replacement consistently in sections 3, 4, 6, and 9, and that `session_compact` is not;
- confirm the capability manifest fields in section 4 match the manifest shape in `docs/PROTOCOL.md` Adapter capabilities (actual manifest fields, not delivery outcomes like `unsupported_command`);
...
- confirm the replacement-window verification criterion (all subscribed surfaces reconcile the generation bump; late old-generation frames become `stale_event` audit-only) is present in section 8 and/or Risks.
...
- **Replacement window, multi-surface fan-out, and `/reload` (review finding I3).** The generation-bump mapping cites `LateGenerationInert`, but three concrete hazards are not yet captured as verification criteria: (a) **replacement window** — events/replies in flight when `session_new` fires must reconcile against the tombstoned generation, not the new one; the Pi adapter must report the bump such that the core can reject late old-generation mutations; (b) **multi-surface fan-out** — remote_pi fans out an empty `session_history` to every owner on `_resetSessionForNew` (`index.ts:2331-2342`); in Patchbay every subscribed surface must reconcile the generation bump consistently, not just the sender; (c) **`/reload` exclusion** — the `story-session-replacement-harness` explicitly leaves `/reload` process behavior out of scope (it is a same-session `session_start:reload` against a re-`require`d module, a Pi-process concern). The checklist must NOT claim `/reload` parity unless separately verified. Mitigation: section 8's switch-decision checklist adds a replacement-window verification criterion; the doc states `/reload` is out of v0 Pi adapter parity scope unless a follow-on verifies it.
...
**Reviewer**: fresh-context cross-model deep review on `openai-codex/gpt-5.5` (high thinking), dispatched by the umans orchestrator. Advisory pass on the design body (no implementation existed yet); no substrate side-effects, no stage change.

**Blockers**: none.
**Important findings** (all applied to the design body in-stride):
- **I1 — manifest column not consistently manifest-shaped.** Section 4's "manifest capability declaration" column held non-manifest values (`(delivery)`, `(read lifecycle)`, `unsupported_command`). Fixed: split per-Operation delivery mapping from manifest-field declarations; the manifest column now uses actual manifest fields (`supported_operation_kinds` includes/excludes, `streaming`, `snapshot`, `cancellation`, `session_replacement`, `idempotency_strength`, `attachment_method`, `known_failure_modes`); `unsupported_command` is moved to delivery-outcome prose.
- **I2 — pairing/queue messages need one consistent lane.** `pair_request` appeared in the `attach` mapping but was later classified as transport/pairing out-of-scope. Fixed: `attach` now maps from `session_sync` only, with an explicit note that `pair_request` is transport/pairing (section 7); the switch-decision checklist (section 8) now requires an explicit accept-or-replace decision for the Remote Pi queued-message behavior (`queued_message_set`/`queued_message_clear`).
- **I3 — replacement-window, multi-surface fan-out, `/reload` limits not explicit.** The `LateGenerationInert` citation covered late-generation inertness but not the in-flight replacement window, multi-surface fan-out of the bump, or the `/reload` harness exclusion. Fixed: added a Risks bullet covering all three (with `index.ts:2331-2342` fan-out citation and the `story-session-replacement-harness` `/reload` exclusion) and a section-8 replacement-window verification criterion; `/reload` parity is out of v0 scope unless a follow-on verifies it.

**Nits** (both applied):
- **N1** — "`session_new` != spawn" rephrased from "does not spawn a new process" to "does not provision a new Patchbay runtime/daemon slot," with a precise note that daemon mode may restart the Pi RPC child without `--continue` (`EXIT_DAEMON_FRESH_SESSION`, `index.ts:2216`) as the mechanism realizing the replacement, not a `spawn`.
