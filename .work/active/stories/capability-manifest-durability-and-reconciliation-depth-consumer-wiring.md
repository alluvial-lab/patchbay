...
- [ ] `AdapterCapabilitySummary` carries the generated assurance message once; no flattened six-field copy or local Rust/TypeScript registry exists.
- [ ] Canonical diagnostics use the validated normalized assurance view; a historical v0.2 registration displays complete conservative V1 and never leaks deprecated/raw fields.
- [ ] Fresh attach, replay, same-generation redeclaration, and newer-generation replacement preserve exact assurance values and existing atomic lifecycle behavior.
- [ ] Pi and token-commune constructors explicitly set every V1 field; deleting any field fails their manifest/attach test.
- [ ] Current Pi does not advertise continuation proof, cursor support, generation fencing, or stronger reconciliation solely from session replacement, `get_entries`, attachment, or reachability.
- [ ] The dependent Pi profile consumes this generated registry and has at most one separate bounded opaque profile for Pi-only facts; generic assurance fields are not redefined there.
- [ ] CLI JSON and human output show safe canonical names. Web retry safety reads V1 dedup and continues to distinguish `execution_outcome_unknown` from ordinary pre-execution retry.
- [ ] Unknown/manual-required presentation derives from existing unknown outcomes plus generated `ReconciliationAction`; no new command/session/failure state or CSS registry member appears.
- [ ] A maximal manifest cannot authorize or complete an Operation, and a conservative manifest cannot suppress otherwise authorized adapter delivery; the exact advisory-only conformance check passes.
- [ ] Attach/reachability/runtime diagnostic state changes do not mutate assurance values.
- [ ] Raw attachment descriptors, future opaque adapter-profile bytes, paths, credentials, and other canonical redaction-list fields remain absent from diagnostics/audit/snapshots.
- [ ] Rolling docs update the canonical adapter-capability registry and extension-seams row without framing the v1 declaration as timeless architecture.
