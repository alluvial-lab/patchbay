...
### Fix round 3 — explicit managed-lineage checkpoint provenance

- Execution capability: `openai-codex/gpt-5.6-sol` (caller-selected for the security-critical recovery boundary); review weight: `thorough` (autopilot caller). Direct-read implementation replaced the disproven inference model rather than adding another checkpoint-shape heuristic.
- Root-cause model: `SessionRegistry` now records managed logical-target ids when it folds accepted claims/staged successors and records the exact logical owner of every session tombstone created by `SpawnPromotionCommitted`. The production checkpoint writer serializes that independent provenance in a private format-3 payload sidecar; format-2 remains readable as unmarked legacy input. This is private recovery serialization only: no `.proto`, generated contract, or public wire shape changed.
- Hydration rule: an explicit marker is the only managed/legacy discriminator. Every marked tombstone must agree exactly across marker, session tombstone, logical-target tombstone, logical target id, identity tuple, generation, and superseding LSN before its current successor is accepted. Unmarked session-only history uses the legacy same-runtime path even after explicit initial-current adoption. An unmarked logical-target tombstone is treated as a disposable cross-version managed shape and deterministically falls back to LSN-0 replay; tombstone presence/current-slot coincidence is never used to infer provenance.
- Regression evidence: both marked asymmetry directions reject; complete marked same-runtime and changed-runtime checkpoints retain reverse ownership; unmarked session-only and replay-reachable legacy-adoption checkpoints hydrate; and the pass-3 old-slot-reuse craft remains rejected even after a later live session occupies the retired native slot. The existing checkpoint recovery regression continues to prove semantic rejection, deterministic full replay, and convergence.
- Mutation evidence: removing managed tombstone provenance from the promotion fold made `continuation_requires_both_live_grants_and_tombstones_n_on_n_plus_one_promotion` fail (exit 101) because the writer-derived symmetry checkpoint lost its marker. Ignoring marker provenance during hydration made `marked_changed_runtime_rejects_missing_logical_tombstone_despite_old_slot_reuse` accept the pass-3 masquerade and fail (exit 101). Each mutant was applied alone and restored from the staged implementation with `git restore`; the clean focused regressions then passed, including the pass-3 legacy-adoption probe.
- Checkpoint version and rolling foundation: format 3 is the current private payload; format 2 decodes as unmarked legacy. `ARCHITECTURE`, `PROTOCOL`, `VERIFICATION`, and `GLOSSARY` now state the explicit-provenance and cross-version fallback contract. The durable log remains authoritative and all incompatibility remains a disposable checkpoint miss.
- Verification: all four prescribed groups pass on the integrated Unit 4 + concurrent Unit 6 tree: `cargo build --workspace --all-targets && cargo test --workspace && cargo clippy --workspace --all-targets -- -D warnings`; contracts drift/vector/model/build (55 vectors, 22 implementation checks, 38 killed mutation witnesses); operator-domain build/tests (23/23); and Pi adapter tests (38/38). Focused checkpoint, promotion, legacy-adoption, old-slot-reuse, and deterministic fallback regressions also pass after both mutations were restored.
- Simplification/discrepancies: removed current-slot/tombstone-presence managed inference entirely. The design named an internal checkpoint payload marker and no protocol change; the repository's private payload happened to reuse a generated message, so the marker is carried by a local Prost format-3 wrapper instead of changing that public/generated schema. Adjacent issues parked: none.
