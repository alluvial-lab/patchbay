...
- Deep-review repair: the production submit path now rejects decoded continuation intents as canonical `unsupported_command` before Grant evaluation, target resolution, or append while fresh spawn remains enabled. This temporary guard is intentionally removed only when `fleet-spawn-target-resolution` owns compound Grant selection.
- Boundary hardening: the raw `SpawnRequest` scanner now admits only tags 1/2/3 with length-delimited wire types and rejects unknown fields plus either mixed-intent wire order. Continuation provenance validation moved to neutral `core/src/contract_validation.rs`; acceptance and authority replay consume it without routing the authority contract through acceptance.
- Mutation evidence: removing the live continuation guard fails `continuation_rejects_before_grant_target_or_append_while_fresh_spawn_remains_enabled`; restoring unknown-field tolerance fails `spawn_wire_rejects_unknown_top_level_fields_for_every_intent`; making mixed-intent detection order-sensitive fails `spawn_wire_rejects_both_intents_in_either_wire_order`; removing the authority replay validation call fails `continuation_descendant_replay_validates_and_preserves_compound_provenance`; admitting `query` as replacement authority fails `compound_authority_requires_both_distinct_grants_and_exact_prior`.
- Review-fix verification: `npm run check:drift && npm run check:vectors && npm run check:models`; `cargo build --workspace --all-targets`; `cargo test --workspace`; and `cargo clippy --workspace --all-targets -- -D warnings` are green. Stage remains `review` for reviewer re-run.
