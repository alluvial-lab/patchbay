...
### Leaf 6 round-5 security BLOCKER convergence — 2026-08-14T15:21:40-06:00

- Execution capability: `openai-codex/gpt-5.6-sol` — caller-selected strongest worker for the security-critical storage exclusivity, durable pre-state, and retry reconciliation boundaries. Review weight remains caller-selected `thorough`; this pass advances only to `review` for the requested independent round-5 review.
- BLOCKER 1 — exclusive transition-producing Observation storage: `AuditedStorage` and `RusqliteStorage` now decode generic `Observation` payloads and reject every status/Result for which the canonical acceptance derivation produces a command transition. The fence applies to append, audited append, decision append, batch-audited append, decision-with-many-audits, and dedup, including direct inner-backend calls. Successful spawn Results use their existing dedicated deferred-evidence transaction; diagnostics Results now use the dedicated atomic Observation+transition boundary rather than a generic source append plus split transition. Probe mapping: `transition_observations_are_exclusive_from_every_generic_storage_route` exercises status, successful Result, and failed Result across the full route matrix and proves a zero-length durable prefix.
- BLOCKER 2 — transactional durable command pre-state: the SQLite Observation+transition writer opens one transaction, reads and replay-validates that transaction's exact durable prefix, rebuilds `CommandIndex`, and checks command existence, exact command correlation, target equality, exact durable `from_state`, the derived outcome/failure, legal adjacency, replay position, and canonical source/transition/audit framing before any insert. It stages both new records through the command projection, then commits Observation → transition → transition-linked audit atomically. Probe mapping: `atomic_transition_append_validates_durable_prestate_and_reconciles_exact_retries` covers missing command, wrong `from_state`, disallowed edge, wrong target, forged command correlation, no-write rejection, exact canonical retry, changed evidence, and file-reopen restart; the diagnostics query lifecycle and authenticated transition-fault tests exercise the same boundary through production paths.
- BLOCKER 3 — exact failed-Result retry reconciliation: retry reconciliation now runs before late-terminal rejection at core acceptance and authenticated adapter ingress. The storage scan recognizes only a complete canonical Observation → transition → audit triplet at the correct replay position and returns the original Observation source ID for an exact terminal retry; changed evidence does not reconcile, and the acceptance boundary rejects it without writing. The dedicated atomic append likewise rejects a non-exact retry against terminal durable pre-state; deferred-success conflicts retain the typed `ObservationEvidenceConflict` path. Probe mapping: the atomic storage probe asserts canonical Observation/transition/audit ID reuse and stable event count across reopen; `authenticated_failed_result_retry_reuses_canonical_source_across_driver_restart` proves the original source ID is returned before and after completion-driver restart; `promotion_producer_treats_exact_failed_result_retries_as_one_non_success` proves retry reconciliation does not turn failure into promotion authority.
- Mutation kills: (1) inverting transition-producing Observation detection in both generic storage fences made `transition_observations_are_exclusive_from_every_generic_storage_route` fail on the first generic append; (2) disabling the durable command-target equality check made the wrong-target no-write assertion in `atomic_transition_append_validates_durable_prestate_and_reconciles_exact_retries` fail; (3) disabling exact-prefix reconciliation made that probe fail with `Observation transition from_state disagrees with durable command spawn-a` instead of returning canonical IDs. Every mutant exited non-zero in its focused test, was reverted with `git restore`, and no mutant was committed.
- Files changed for the blockers: `core/src/acceptance/{mod,observation}.rs`; `core/src/storage/{port,audited,rusqlite}.rs`; `core/tests/{acceptance_observation,authority_ingest,authority_proptest,runtime_evidence_promotion}.rs`; `server/src/{adapter_service,service,state}.rs`; `server/tests/spawn_completion.rs`. Historical stranded-source fixtures now use explicit backend-only SQL so production generic routes remain closed.
- Verification harness alignment: the workspace packages now declare the requested empty `proptest` feature gate, and `formal/run-model-checks.sh` enforces model-promotion metadata, typechecks all 11 Quint models, and executes the nine promoted invocation records from their `@promotion` blocks. `cargo fmt --all` normalized the pre-existing workspace formatting drift so the required formatter group is an actual green gate.
- Simplification and compatibility: the implementation reuses the existing SQLite single-writer actor, full generated `Observation` equality, replay validator, `CommandIndex`, and audit framing; it adds no wire field, protobuf change, generated-contract change, side table, repair path, or alternate transition producer. Existing round-1 through round-4 staging, quarantine, promotion, authority, and aggregate-fold fixes remain intact.
- Verification group 1 — `cargo test --workspace`: **PASS**.
- Verification group 2 — `PROPTEST_CASES=256 cargo test --workspace --features proptest`: **PASS**.
