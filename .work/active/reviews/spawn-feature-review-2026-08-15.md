...
## M1 fixed — promotion-sourced command diagnostics (2026-08-16)

MATERIAL 1 is fixed in core diagnostics. `DiagnosticsProjection` now retains a canonical `CommandIndex` fold and requires every durable event, including `SpawnPromotionCommitted`, to pass that existing command projection before publishing diagnostic state. The promotion handler therefore consumes `CommandIndex`'s exact operation/grant/pre-state/deferred-result semantics instead of copying promotion validation. A valid promotion appends `completed` to the same command timeline with the promotion envelope as `terminal_event_id` and history source.

Spawn-claim disposition changes are no longer inert: the claim handler tracks the generated disposition adjacency through the shared registry rule and appends timeline evidence. Poisoned or abandoned claims use the existing `execution_outcome_unknown` diagnostic vocabulary without changing canonical command state; release remains a non-failure claim marker; promotion records both `promoted` internally and the terminal `completed` history entry. No protocol shape changed.

The server integration now asserts hot and exact bounded-as-of command inspection before and after promotion plus restart inspection after promotion. The post-promotion assertions require `completed`, unspecified failure, and the exact promotion event as terminal/history source. Mutation testing made the promotion diagnostics match arm inert; the focused server regression failed (`delivered` instead of `completed`, exit 101), and `git restore --worktree core/src/diagnostics/mod.rs` reinstated the staged fix. Direct reading was sufficient for this bounded ownership surface; execution capability was `openai-codex/gpt-5.6-sol`, with caller-selected thorough feature re-review to follow.

Full verification passed: `cargo build --workspace --all-targets && cargo test --workspace && cargo clippy --workspace --all-targets -- -D warnings`; contract drift/vectors/models/build; operator-domain build/tests; Pi adapter tests; and the complete CLI suite including its real-core resource projection. The first parallel CLI attempt overlapped operator-domain's clean rebuild and transiently could not resolve that package; the required sequential rerun passed 48/48 plus the real-core check.

## M3 fixed — authenticated atomic target abandonment (2026-08-16)

MATERIAL 3 is fixed as a complete operator recovery lifecycle. The generated `ControlService.AbandonSpawnTarget` contract selects one exact accepted claim and logical target but carries no self-asserted issuer or authority. Core authenticates the compound operator issuer, catches every projection up under the shared decision gate, derives the Grant scope from durable claim state (exact prior runtime for continuation, accepted adapter for fresh spawn), and checks a live `SESSION_MANAGEMENT` Grant. Grant liveness, typed abandonment evidence, and audit share one sampled instant. Denials are audited; malformed, unknown, mismatched, non-active/non-poisoned, retired, or live-successor cases fail before abandonment mutation.

The SQLite writer is the sole append route. Inside one writer transaction it replays claim, command, and session/target pre-state; validates exact claim/current/candidate ownership and canonical audit scope; stamps the source event id into `SpawnClaimAbandonmentEvidence`; stages both claim and session folds; and commits the source plus its immediately linked `SPAWN_TARGET_ABANDONED` audit atomically. A semantic-exact retry returns the original source/audit ids without another write, while a changed terminal retry conflicts. Generic `SpawnClaim` append routes remain rejected.

Replay now makes the decision operational: the claim becomes permanently consumed, its continuation fence opens, the logical target receives a durable retirement LSN, current and reserved-candidate identities move to retained audit-only tombstones with reverse ownership, and current session state is tombstoned. No initial assignment, reservation, staging, or promotion can revive the target; checkpoint/restart preserves retirement, including migration of the legacy empty-slots-plus-tombstones checkpoint shape. Command and diagnostics projections observe the same source event.

CLI exposes `spawn-target-abandon`; the authenticated web bridge applies normal CSRF/session checks; and the cockpit carries the claim's logical-target identity, offers the recovery action only for active/poisoned claims, renders the canonical generated disposition, and tombstones matching session views after the durable event. The disposition label is owned once in `operator-domain` and reused by both surfaces.

Regression and mutation-witness coverage includes unauthenticated requests; wrong-kind, wrong-target, revoked, and expired Grants; wrong logical target and audit scope; non-exact retry after terminal abandonment; protected generic append rejection; source/audit adjacency and sampled-time equality; exact retry idempotency; current/candidate audit-only retirement; fence clearing; no revival; hot/restart/checkpoint equality; command/diagnostic visibility; web CSRF forwarding; cockpit action/removal; and CLI typed output. Full Rust, 256-case property, 20/20 formal, contracts/vector/model/presentation, operator-domain, web server, web cockpit, CLI, Pi adapter, token-commune adapter, both core smokes, and composed walking-skeleton verification passed. The generated-drift checker is rerun after the intended generated contract changes are committed, because it correctly rejects a dirty generated diff. MATERIAL 3 has no remaining implementation blocker; the review's separate MATERIAL 4 retained-review work and the explicitly deferred concrete Pi spawn supervisor remain outside this fix.
