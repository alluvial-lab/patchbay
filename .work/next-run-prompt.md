# UAT IN PROGRESS (2026-08-17) — ALL 7 UAT bugs FIXED; first full happy path VERIFIED LIVE (spawn→handshake→staged→promoted→live child)

Bug ledger: (1) cockpit AcceptedOperation decode `28868d1`; (2) CLI sibling `5f483b1`/`101860e`; (3) spawn delivery wiring `37d0c52`; (4) spawn payload shape from declared capability `b5a9f33`; (5) diagnostics infinite loop `22c5aee`+`d37f8d7`; (6) empty-state capability fetch `80e8807` (also annotates empty Resources destination); (7) supervisor session-directory root cause `adec4c11` (handshake was FINE — Pi defaulted --session-dir outside sessionRoot; now sessionRoot defaults it; local diag failure codes added). Live verification 2026-08-17 ~09:48: spawn accepted→delivered→running→child launched→handshake OK→claim PROMOTED→command completed→session LIVE (live/idle, zai/glm-5.3, child ppid=adapter). Still open (design decisions, not bugs): `fix-fresh-spawn-one-shot-managed-target` (options a/b/c).
