...
use patchbay_core::{acceptance::target_key_for, resource::{ResourceIdentity, ResourceIdentityError}};
use prost::Message;
...
fn canonical_identity_round_trips_and_each_tuple_dimension_is_distinct() {
    ...
    for value in &variants {
        assert_eq!(ResourceIdentity::try_from_scope(&value.to_scope()).unwrap(), *value);
    }

    let encoded: HashSet<_> = variants.iter().map(|value| value.to_scope().encode_to_vec()).collect();
...
fn empty_partial_mixed_legacy_and_dual_shapes_are_rejected() {
    ...
    assert_eq!(
        ...
        Err(ResourceIdentityError::Missing { field: "resource_kind" })
    );

    let mut mixed = identity("a", "pool", "one").to_scope();
    mixed.adapter_id = Some(AdapterId { value: "a".to_owned() });
    assert_eq!(
...
fn protobuf_tag_eight_still_decodes_as_audit_only_data() {
    ...
    let bytes = [0x08, TargetScopeKind::Resource as u8, 0x42, 0x06, b'l', b'e', b'g', b'a', b'c', b'y'];
