...
fn sibling_coverage_requires_valid_contiguous_framing_and_owned_payload() {
    ...
    registry
        .observe(&recorded(&domain, 2, state_event(&domain, &identity)))
        .unwrap();
...
fn replay_ignores_sibling_event_kinds_without_hiding_resource_state() {
    ...
    assert!(rebuild_from_events(&domain, &prefix)
        .unwrap()
        .contains(&identity));
}
...
fn opaque_observation_dispatch_mutant_is_killed() {
    ...
    let decoded =
        ResourceStateEvent::decode(encoded_state.as_slice()).expect("plausible injected bytes");
    let mutant = rebuild_from_events(&domain, &[recorded(&domain, 1, decoded)]).unwrap();
    assert!(
        mutant.contains(&identity),
        "the independent no-core-state oracle kills the dispatch mutant"
    );
}
