...
pub fn audit_draft_for_source(
    ...
    match kind {
        StoredEventKind::Operation => {
            let accepted =
                AcceptedOperation::decode(payload.payload.as_slice()).map_err(|error| {
                    StorageError::CorruptRecord(format!(
                        "cannot decode accepted operation for audit: {error}"
                    ))
                })?;
        ...
        StoredEventKind::CommandTransition => {
            let transition =
                CommandTransition::decode(payload.payload.as_slice()).map_err(|error| {
                    StorageError::CorruptRecord(format!(
                        "cannot decode command transition for audit: {error}"
                    ))
                })?;
            ...
            draft.kind =
                match patchbay_contracts::patchbay::OperationState::try_from(transition.to_state)
                    .ok()
                {
        ...
        StoredEventKind::Observation => {
            ...
            draft.actor_id = observation
                .sender
                .as_ref()
                .and_then(|sender| sender.actor_id.clone());
            draft.endpoint_id = observation
                .sender
                .as_ref()
                .and_then(|sender| sender.endpoint_id.clone());
            draft.device_id = observation
                .sender
                .as_ref()
                .and_then(|sender| sender.device_id.clone());
            ...
            let schema = observation
                .payload
                .as_ref()
                .map(|payload| payload.schema_ref.as_str());
            draft.kind = if schema == Some("patchbay.AdapterRegistration") {
                ...
                match ObservationKind::try_from(observation.kind).ok() {
                    Some(ObservationKind::Status) if draft.command_id.is_some() => {
                        AuditEventKind::CommandRunning
                    }
                    Some(ObservationKind::Result)
                        if observation.failure_code == FailureCode::Unspecified as i32 =>
                    {
                        AuditEventKind::CommandCompleted
                    }
        ...
        StoredEventKind::OperatorRecord => {
            let record = OperatorRecord::decode(payload.payload.as_slice()).map_err(|error| {
                StorageError::CorruptRecord(format!(
                    "cannot decode operator record for audit: {error}"
                ))
            })?;
        ...
        StoredEventKind::ControlSurfacePrincipal => {
            ...
            .map_err(|error| {
                StorageError::CorruptRecord(format!("cannot decode principal for audit: {error}"))
            })?;
        ...
        StoredEventKind::OperatorSessionRevocation => {
            let revocation = patchbay_contracts::patchbay::OperatorSessionRevocation::decode(
                payload.payload.as_slice(),
            )
            .map_err(|error| {
                StorageError::CorruptRecord(format!(
                    "cannot decode operator-session revocation for audit: {error}"
                ))
            })?;
            draft.actor_id = revocation.operator_actor_id;
            draft.endpoint_id = revocation
                .verified_revoker
                .as_ref()
                .and_then(|value| value.endpoint_id.clone());
            draft.device_id = revocation
                .verified_revoker
                .as_ref()
        ...
        StoredEventKind::ControlSurfaceRevocation => {
            let revocation = patchbay_contracts::patchbay::ControlSurfaceRevocation::decode(
                payload.payload.as_slice(),
            )
            .map_err(|error| {
                StorageError::CorruptRecord(format!(
                    "cannot decode control-surface revocation for audit: {error}"
                ))
            })?;
            ...
            draft.kind = match revocation.target.as_ref() {
                ...
                Some(
                    patchbay_contracts::patchbay::control_surface_revocation::Target::DeviceId(id),
                ) => {
                    draft.target_scope = Some(patchbay_contracts::patchbay::TargetScope {
                        kind: patchbay_contracts::patchbay::TargetScopeKind::Resource as i32,
                        legacy_audit_resource_id: id.value.clone(),
        ...
        StoredEventKind::SecurityLockdown => {
            let source =
                SecurityLockdownEvent::decode(payload.payload.as_slice()).map_err(|error| {
                    StorageError::CorruptRecord(format!(
                        "cannot decode security lockdown for audit: {error}"
                    ))
                })?;
            match source.transition {
                Some(
                    patchbay_contracts::patchbay::security_lockdown_event::Transition::Entered(
                        entered,
                    ),
                ) => {
                    draft.kind = AuditEventKind::LockdownEntered;
                    draft.actor_id = entered
                        .entered_by
                        .as_ref()
                        .and_then(|value| value.actor_id.clone());
                    draft.endpoint_id = entered
                        .entered_by
                        .as_ref()
                        .and_then(|value| value.endpoint_id.clone());
                    draft.device_id = entered
                        .entered_by
                        .as_ref()
                        .and_then(|value| value.device_id.clone());
                ...
                Some(
                    patchbay_contracts::patchbay::security_lockdown_event::Transition::Exited(
                        exited,
                    ),
                ) => {
                ...
                None => {
                    return Err(StorageError::CorruptRecord(
                        "security lockdown has no transition".to_owned(),
                    ))
                }
            }
        }
    ...
    S: Storage + Clone,
{
    ...
    async fn append_dedup_with_payload(
        ...
        match self
            .inner
            .append_dedup_audited_with_payload(
                authority_domain_id,
                key,
                target,
                payload,
            ...
            .await?
        {
            AuditedDedupOutcome::Appended(result) => {
                Ok(DedupOutcome::Appended(result.source_event_id))
            }
            AuditedDedupOutcome::Duplicate {
                source_event_id, ..
            } => Ok(DedupOutcome::Duplicate(source_event_id)),
        }
    }
    ...
    async fn append_quarantined_runtime_evidence_audited(
        ...
        self.inner
            .append_quarantined_runtime_evidence_audited(authority_domain_id, quarantined, audit)
            .await
    }
    ...
    async fn append_decision_audited_many(
        ...
        reject_generic_special(&source)?;
        self.inner
            .append_decision_audited_many(authority_domain_id, source, audits)
            .await
    }
    ...
    async fn query_audit_through(
        ...
        self.inner
            .query_audit_through(authority_domain_id, spec, as_of_lsn)
            .await
    }
}
