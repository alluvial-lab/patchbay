...
Restart presentation reuses canonical spawn lifecycle, failure, claim-poison, retry-risk, and descendant-Grant facts; it does not invent a restart state. Logical-context status is exactly `resumed`, `new_context`, or `unknown`. The copy must explain that this is adapter-native context continuity and never claim arbitrary process memory, tool state, or in-flight execution was restored. N+1 is not shown current until the atomic promotion event is folded; during ambiguity the prior is stale/offline/failed with unknown activity and the successor remains staged/unpublishable.
