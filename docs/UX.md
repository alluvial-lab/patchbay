...
- **Identity-before-intent.** Show stable target identity before the operator can submit an Operation. Runtime sessions use adapter id, deployment scope, runtime session id, and session generation; operational resources use their adapter/resource identity without fabricating a runtime session or generation. Human-readable labels (project, cwd, name, provider label, pool name) are metadata, not identity — they must not override verified target identity.
