...
- **Failure vocabulary mapping.** Map failure text to the protocol failure/outcome vocabulary in `docs/PROTOCOL.md` so timeout, denial, rejection, expiration, cancellation, supersession, and execution failure remain distinct. Show what is safe to retry. **Retry safety is derived from the specific failure term plus the adapter's generated `assurance.v1.deduplication_strength`, never from `CommandState` or capability alone.** `failed` includes `execution_failed` (the target began or accepted execution and reported failure) and `execution_outcome_unknown` (execution may have occurred); neither is unconditionally safe to retry. The surface presents retry safety by combining the failure term with the declaration:

  | Failure term | `assurance.v1.deduplication_strength` | Retry safety |
  |---|---|---|
  ...
  The surface must never present a retry as unconditionally safe without these signals. An **intentional duplicate** (a distinct new action, not a retry) is presented as a new action requiring a new command id and a new idempotency key, never as a retry of the original. When the canonical result is `SubmissionOutcome = unknown` or `CommandState = failed` with `execution_outcome_unknown`, generated `unproven_outcome_action = none` adds the closed qualifier `unknown`, while `manual_required` adds `manual-required`. The qualifier never rewrites the canonical outcome, and no new command/session/failure state or CSS registry member is created.
...
The composer surfaces the in-session OperationKinds: `instruct` with prompt payload, `cancel`/`interrupt`, `approval-response`/`elicitation-response`, `query`, `reconfigure`, and `session-management`. `spawn` and `attach` are surfaced as entry-point actions elsewhere (per the Operation affordance coverage obligation). Session-list spawn creates a fresh spawn Operation for one explicit adapter and selected configured managed target; session-detail restart creates another spawn Operation with a new command/key and the exact current managed continuation. Both actions derive the target-spec shape and intent-specific opaque adapter payload from the adapter's canonical capability summary. They remain disabled with a canonical explanation when capability is unavailable, `spawn` is undeclared, the shape set is zero/multiple, or no exact managed target is selected. Restart additionally remains unavailable when the surface cannot reconstruct the managed logical-target identity, the target is superseded/unreconciled, or lockdown applies. The composer displays local submission state and durable `CommandState` using the canonical registries in `docs/PROTOCOL.md`, and shows idempotency behavior on retry.
...
| `adapter-status [adapter-id ...]` | `--after-adapter-id`, `--limit 1..500`, `--json` | Redacted adapter status, complete generated assurance V1 dimensions, the `unknown`/`manual-required` qualifier, recent diagnostics, and `{ hasMore, nextAfterAdapterId }`; omitted limit uses the core default of 100 and the maximum is 500. |
...
- Retrying commands without showing deduplication behavior or retry safety (per the `assurance.v1.deduplication_strength` + canonical failure matrix), or deriving a retry decision from capability alone.
