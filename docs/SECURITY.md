...
A fresh spawn uses only that live adapter-scoped `spawn` Grant. Intentional restart is a new spawn command/key with an exact continuation payload, not a privileged restart RPC. A continuation additionally requires a live Grant for the same verified subject, endpoint, and authority domain, scoped to the exact prior logical-target/runtime generation and allowing `session-management`. The two Grants are selected under one decision gate and sampled decision time. Durable continuation provenance preserves the exact prior reference, the adapter-spawn Grant id, the exact-prior replacement Grant id, and the canonical `session-management` kind. Missing, malformed, mismatched, expired, or revoked replacement authority fails closed; adapter-wide spawn authority alone cannot replace or revive a protected generation.

Continuation staging and promotion are additionally fenced by the session projection: the exact prior must still be current for its logical target, connectivity must be `offline`, `stale`, or `failed`, and activity must be `unknown`. Thus the adapter cannot publish a successor while prior N still appears available or active. A clean old-runtime outcome leaves N current-but-unavailable until the atomic promotion; stream loss degrades N to stale/unknown and ambiguous post-offer/launch evidence poisons the claim. Poison retains the fence and reservation, suppresses automatic relaunch, and requires later exact handshake/success evidence before reconciliation can promote.
...
A single core owner folds the gap-free LSN prefix and derives a managed promotion only after exact accepted claim/provenance, delivered/running-before-success lifecycle, and the durable exact-claim phase chain agree. Fresh spawn requires `external_identity_known → handshake_reconciling → staged → success_evidence_reported`; continuation additionally requires `delivered → quiescing_prior → prior_terminated → launch_attempted` before that suffix. After poison, handshake and success must be later than the poison decision. The generated adapter-reported logical-context registry is carried from the exact continuation report into staged/promoted evidence as `resumed`, `new_context`, or `unknown`; core never invents it, and it never proves restoration of arbitrary process state. It consults the durable authority projection at the promotion's sampled time: the accepted spawning Grant must still authorize the adapter target, and continuation requires the same accepted exact-prior replacement Grant to remain live and exact-session scoped for the verified actor/endpoint. Revocation or expiry suppresses promotion rather than laundering authority through a descendant Grant.
