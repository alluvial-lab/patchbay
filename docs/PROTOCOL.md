...
- A derived snapshot is compatible only when the storage row's `EventId` has the current authority domain and an LSN, the embedded authority domain and nonzero core generation exactly equal the current durable values, and the embedded snapshot LSN exactly equals the storage-row LSN. Missing/zero/mismatched anchors or undecodable payloads make the checkpoint unusable and the core repairs from current log materialization or full replay. Compatibility does not establish freshness: an otherwise compatible older snapshot remains rejected by the separate per-view LSN rule above.
...
- **Snapshot materialization**: the store is snapshot-capable and `LoadSnapshot` materializes an on-demand snapshot at the current `LSN`; periodic checkpoint writing to bound replay cost is not implemented — recovery replays the durable log. Stored session checkpoints are disposable derived data and must pass the exact authority-domain/core-generation/LSN compatibility rule before use. Snapshots remain derived artifacts, never an alternate source of truth.
...
| session runtime metadata | opaque adapter-reported current `model` with registration/generation/snapshot carriage and durable `SessionModelChanged { identity, from, to }` delta | C | `feature-session-model-field`; PROTOCOL "Session state axes" |
