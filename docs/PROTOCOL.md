...
3. record typed quiesce/old-runtime evidence and make N's activity explicitly `unknown`;
4. let the adapter terminate, replace, and reconcile through its declared implementation;
5. stage the exact successor report and reserve its external identity without publishing it;
6. retain successful Result plus ordered `external_identity_known → handshake_reconciling → staged → success_evidence_reported` readiness evidence; and
...
Adapter logical-context reporting uses only `resumed`, `new_context`, or `unknown`. These values describe adapter-native logical context; none means that Patchbay restored arbitrary process memory, file descriptors, tool state, or in-flight execution.
