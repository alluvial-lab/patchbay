...
| observability / diagnostics | durable queryable audit log + CLI `audit-query`/`inspect-command`/`adapter-status`; v0.1.0 CLI stubs reject with a non-zero prerequisite message pending core-diagnostics | R | SPEC v0.1.0 observability scope; UX CLI; SECURITY Audit events |
