...
- **One `spawn` OperationKind with disjoint generated intents.** `SpawnRequest.intent` is exactly one of `fresh` or `continuation`; worktree, same-dir, session, process, thread, local sidecar, and cloud-environment spawns are not separate OperationKinds in v0.1.0. Intentional restart is a new `spawn` Operation with a new command id/idempotency key and a continuation intent, never a `restart` kind or a hidden `session-management` mutation. A continuation carries one exact `RuntimeGenerationRef` for the prior logical target and external runtime. Missing/wildcard identity, generation zero, and a prior generation that cannot advance reject as `validation_failed` before stateful work. Per-variant OperationKinds are reserved only if a future registry update promotes them.
...
- **Deduplication is assurance-manifest declared.** Spawn uses `AdapterCapability.assurance.v1.deduplication_strength` (`none` / `at-Patchbay-boundary` / `end-to-end`). Patchbay-boundary strength means Patchbay deduplicates the Operation record while adapter retry may still create a duplicate external process. `end-to-end` requires adapter evidence across response loss and restart. Duplicate external-process reporting maps through the failure vocabulary; Patchbay does not infer adapter-side prevention from attachment, session replacement, or core boundary dedup.
...
| `execution_outcome_unknown` | execution | The target may have begun or completed execution, but Patchbay cannot determine the outcome (e.g. adapter crash after execute-before-ack, transport loss after delivery). The command transitions to `failed`; the ambiguity is surfaced to control surfaces so retry safety can combine this canonical failure with `assurance.v1.deduplication_strength`. | `failed` (with ambiguity signal) |
...
The committed generic restart-as-continuation order composes those evidence cells without defining adapter process mechanics:

1. accept the exact claim plus compound provenance and activate the prior-N delivery fence;
2. deliver that accepted claim and target specification to the selected adapter;
3. record durable exact-claim `quiescing_prior → prior_terminated` evidence after delivery and make N's activity explicitly `unknown`;
4. let the adapter terminate, replace, and reconcile through its declared implementation, retaining `launch_attempted → external_identity_known → handshake_reconciling` in that order;
5. stage the exact successor report only after that durable prefix and reserve its external identity without publishing it;
6. retain successful Result plus a later `success_evidence_reported`; and
7. let the sole completion driver atomically promote, or leave the claim active/poisoned for explicit reconciliation.

For continuation, staging and promotion additionally require exact prior N to remain the logical target's current generation while connectivity is `offline`, `stale`, or `failed` and activity is `unknown`. A clean old-runtime outcome therefore leaves N current-but-unavailable until promotion; only `SpawnPromotionCommitted` tombstones N and publishes N+1. Stream loss sets the affected session to stale/unknown and ambiguous post-offer/launch evidence poisons the claim; no failure path allocates or publishes N+1. After poison, an old handshake/success suffix is insufficient: explicit reconciliation must append a later exact handshake and success for the same reserved runtime.

The generated `ContinuationContextStatus` registry contains only `resumed`, `new_context`, or `unknown` plus the wire-required `unspecified` sentinel. For an exact continuation successor, the adapter must choose a non-sentinel value in its `SessionReport`; the staged evidence copies that exact value, promotion preserves it, and operator projections derive it only from staged/promoted evidence. Fresh and ordinary reports must use `unspecified`, and the core never defaults a continuation outcome. The three reported values describe adapter-native logical context; none means that Patchbay restored arbitrary process memory, file descriptors, tool state, or in-flight execution.
...
Adapters that cannot guarantee idempotent external execution set `assurance.v1.deduplication_strength` to `none` or `at-Patchbay-boundary` as applicable. Patchbay still deduplicates at the coordination boundary and exposes the adapter limitation to control surfaces. A surface derives retry safety from the canonical failure term plus this declaration; capability alone never creates a retry decision.
...
- **Snapshot materialization**: the store is snapshot-capable and `LoadSnapshot` materializes an on-demand snapshot at the current `LSN`. A best-effort production writer checks once per second and targets a complete session checkpoint after each 256 newly observed authority-domain events. A successful write atomically retains one latest non-regressing row per authority domain; failure leaves the prior row and authoritative log unchanged, emits bounded structured stderr evidence, and retries with capped 1-to-30-second backoff. The private format-3 payload restores the complete authority-domain-bound `SessionRegistry` (live records, source cursors, retained generation tombstones, explicit managed-lineage provenance, lockdown clamp, and revisions) only after typed/versioned and exact domain/core-generation/LSN plus semantic validation. A marked managed tombstone must have exact session and logical-target counterparts; unmarked session-only legacy history uses its same-runtime successor rule, while an unmarked logical-target tombstone is disposable cross-version state and forces full replay. Format-2 payloads decode as unmarked legacy checkpoints under those rules. Any incompatibility or post-anchor tail disagreement falls back to full session replay. A seeded registry rejects direct covered-prefix re-feed because compacted prior event bytes are not retained, while tail events retain exact-envelope redelivery checks. Under healthy scheduling the session fold normally replays only the events after the latest checkpoint, but this is not a hard 256-event bound under write failure, starvation, overload, or pre-poll appends, and it is not a whole-core recovery bound because every named sibling projection still full-replays.
...
A successful spawn `Result` is durable completion evidence, not by itself the public terminal transition or permission to redeliver the spawn. One exact-correlation rule qualifies that evidence everywhere: one or more identical non-empty `CommandId` references collapse to the same logical correlation, while empty or conflicting ids reject at observation ingress and cannot enter only the redelivery-suppression side of the decision. Delivery reconstruction suppresses redelivery once qualifying success is durable, including across adapter reconnect/core restart, because repeating a non-idempotent external spawn could duplicate the runtime. The core records a bounded, redacted `CommandRunning/spawn_completion_deferred` audit checkpoint for `inspect-command`; the canonical command remains `delivered` or `running`, with no terminal success claim, while promotion is pending.

The single completion owner derives a managed promotion only from the exact accepted claim and compound provenance, delivered/running lifecycle before the successful Result, and a durable phase chain. Fresh spawn requires `external_identity_known → handshake_reconciling → staged → success_evidence_reported`; continuation additionally requires `delivered → quiescing_prior → prior_terminated → launch_attempted` before that suffix. Post-poison reconciliation may reuse an existing reservation but requires later handshake/success evidence. The owner also requires one staged successor and external-runtime uniqueness reservation, non-released/non-abandoned effect state, and the exact `∅→1` or `N→N+1` projection pre-state. A continuation also requires N to remain exact-current but unavailable (`offline`/`stale`/`failed`) with `unknown` activity. At the same sampled promotion time, the spawning Grant must still authorize the accepted adapter target and a continuation's accepted exact-prior session-management Grant must still be live, exact-scope, and bound to the same verified subject/endpoint. Revocation or expiry suppresses promotion; the owner never substitutes another Grant id into accepted provenance.

While holding the shared decision gate, the owner calls the dedicated storage writer that stamps and atomically commits one `SpawnPromotionCommitted` source plus its immediate `CommandCompleted/spawn_completion` audit. The source is the first durable fact containing the descendant Grant, N tombstone/N+1 current transition, claim consumption, and completed command state. Aggregate hot/replay publication folds authority → session → claim → command and installs all views together. A crash before the transaction leaves no promotion; a lost acknowledgement after commit replays the complete decision. The older audit → separate descendant Grant → separate completed-transition tail is isolated to pre-managed durable histories. Startup normalizes its evidence-only, audit-only, audit+grant, and completed prefixes without duplicating grants or terminal transitions, but every `SpawnClaim` history uses only the managed promotion path. Thus readers using the core decision gate observe either the staged pre-promotion prefix or the complete authority-bearing promotion, never public N+1/completed state without descendant authority.
...
- one complete versioned `AdapterAssuranceManifest` (registry below);
- at most one optional bounded opaque adapter-profile envelope, whose generated adapter-specific payload is not a core behavior registry;
- attachment method (adapter-specific descriptor);
- known failure modes (advisory list mapping to the failure vocabulary).

`AdapterAssuranceManifest` is a generated oneof whose admitted current branch is the frozen V1 registry:

| V1 dimension | Values | Conservative meaning |
|---|---|---|
| `deduplication_strength` | `none`, `at-Patchbay-boundary`, `end-to-end` | `none`; Patchbay boundary dedup alone proves at most `at-Patchbay-boundary`, while `end-to-end` requires adapter conformance across response loss and restart. |
| `continuation_proof_support` | explicit `true` / `false` | `false`; session replacement, a reused id, or a resume flag is not proof of exact prior/successor continuity. |
| `cursor_support` | explicit `true` / `false` | `false`; core LSNs, partial history, and bounded latest-window visibility are not an authoritative external cursor or exact-set replacement path. |
| `generation_fence_support` | explicit `true` / `false` | `false`; reporting a generation or core-side stale-event rejection does not prove an external-runtime fence. |
| `reconciliation_strength` | `none`, `bounded`, `authoritative` | `none`; the adapter-wide declaration is the conservative minimum across its declared externally effecting behavior. |
| `unproven_outcome_action` | `none`, `manual-required` | `none`; for a canonical unknown outcome, `none` presents `unknown` while `manual-required` presents the closed `manual-required` operator qualifier. Neither value rewrites the outcome. |

Fresh attach, same-generation redeclaration, and newer-generation replacement require the V1 branch, non-`UNSPECIFIED` generated enums, and presence for all three optional booleans (including explicit `false`). The deprecated top-level deduplication tag is replay-only and must be `UNSPECIFIED` on fresh input. Durable pre-assurance registration replay normalizes recognized legacy deduplication into V1 and makes every other dimension false/none; unknown legacy deduplication normalizes to `none`. An invalid present V1 is corruption and never enters that compatibility path. Canonical diagnostics project `validated_capability.assurance` back to the same generated V1 message, so fresh declarations and historical replay have one complete consumer shape without exposing the replay-only tag.

The current Pi declaration is evidence-backed and activated: it lists `spawn`, `attach`, `instruct`, `cancel`, `interrupt`, `query`, `reconfigure`, and `session-management`, declares managed target shape `pi-rpc`, session replacement, continuation proof, cursor support, and generation fencing, and retains `partial` session snapshot support. Its V1 assurance is `at-Patchbay-boundary` deduplication, `bounded` reconciliation, and `manual-required` for unproven outcomes. The bounded scope is exact persisted-transcript projection replacement inside a verified Pi continuity scope; it is authoritative for that membership replacement only. Ambiguous externally effecting Operation outcomes—including a launch that may exist before identity is known and a reload whose post-effect result cannot be proved—remain `execution_outcome_unknown` and require manual reconciliation. `patchbay.PiRuntimeProfile.v1` describes the conditional materialization and reload boundary opaquely: memory-only sessions are not resumable or restart-stable, while `require_resume` and reload require strict materialization. The profile still activates nothing by itself; the complete managed-lifecycle evidence gate owns the stronger declaration. The current token-commune observer declares `none`, all three booleans `false`, reconciliation `none`, and action `none`; its partial snapshots and latest-50 event visibility do not claim authoritative external cursor or Operation-outcome reconciliation.
...
The optional adapter profile uses the generic `PayloadEnvelope` carrier. A present profile requires a non-empty payload of at most 64 KiB, a 1–256 byte schema reference without ASCII whitespace/control bytes, and a known non-`UNSPECIFIED` content type. Absence is valid. The core applies exactly this framing validation to every adapter, stores the opaque bytes only as part of registration, and never selects or decodes an adapter-specific schema. Diagnostics expose only `{present, schema_ref, content_type}`; the schema reference includes its version (for example `patchbay.PiRuntimeProfile.v1`). Raw profile bytes and profile-local paths are absent from diagnostics, snapshots, and audits. The adapter-specific generated decoder owns enum presence and `UNSPECIFIED` rejection.
...
- **Committed post-v0.1 adapter direction:** the `AdapterTargetCategory` registry admits `runtime_session` and `operational_resource`; each operational resource kind has an exact snapshot tier and schema-bound payload/domain projection contract. The generated complete `AdapterAssuranceManifest` V1 declares deduplication, continuation proof, cursor, generation fence, reconciliation strength, and unproven-outcome action; fresh declarations fail closed, historical replay normalizes conservatively, and canonical diagnostics/surfaces consume the generated block. Resource state uses typed snapshot/delta reports, current/stale/unknown reconciliation freshness, core-LSN record/view revisions, terminal exact-identity tombstones with atomic distinct replacement, durable `RESOURCE_STATE` replay, and discriminated session/resource snapshot loading. `knowledge_bundle` is wire-present and registration-rejected with OKF v0.2 named as the candidate format. Schema matching is a format binding, not semantic payload validation, and capabilities remain advisory rather than authority.
...
| adapter capabilities | 3-tier snapshot model plus the complete generated `assurance.v1` registry (deduplication, continuation proof, cursor, generation fence, reconciliation, unproven action), explicit uncertainty, conservative replay normalization, and canonical redacted diagnostics carriage | C (post-v0.1 direction) | `capability-manifest-durability-and-reconciliation-depth`; PROTOCOL "Adapter capabilities" |
| adapter capabilities | one optional bounded opaque generated adapter profile; core validates only generic envelope framing/size and diagnostics expose only profile presence/schema-version metadata | C (post-v0.1 direction) | `research-handoff-pi-adapter-capability`; PROTOCOL "Adapter capabilities" |
| adapter capabilities | future assurance branches, per-Operation/per-target overrides, signed declarations, evidence-retention windows, and richer reconciliation workflows | R | `capability-manifest-durability-and-reconciliation-depth`; PROTOCOL "Adapter capabilities" |
| adapter capabilities | capability-derived authority/completion/delivery suppression or inference from runtime liveness, session replacement, partial snapshots, or SDK method names | X | `capability-manifest-durability-and-reconciliation-depth`; PROTOCOL "Adapter capabilities" |
...
| adapter execution idempotency | end-to-end adapter-side execution idempotency (no double-execute on retry); declared via `assurance.v1.deduplication_strength`, not a formal property until a future adapter contract model is scoped | R | PROTOCOL `AdapterAssuranceManifest`; VERIFICATION spawn-idempotency note |
