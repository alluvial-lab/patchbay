...
The current Pi declaration is evidence-backed and conservative: `at-Patchbay-boundary`, continuation/cursor/generation-fence `false`, reconciliation `none`, unproven action `manual-required`, and session replacement `false` until the final managed-lifecycle conformance checkpoint supplies the conditional proof. It carries `patchbay.PiRuntimeProfile.v1` opaquely but does not list `spawn` or a managed target shape; the profile contract alone activates nothing. The current token-commune observer declares `none`, all three booleans `false`, reconciliation `none`, and action `none`; its partial snapshots and latest-50 event visibility do not claim authoritative external cursor or Operation-outcome reconciliation.
