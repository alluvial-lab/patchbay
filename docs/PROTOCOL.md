...
The single completion owner derives a managed promotion only from the exact accepted claim and compound provenance, delivered/running lifecycle before the successful Result, one staged successor and external-runtime uniqueness reservation, non-released/non-abandoned effect state, and the exact `∅→1` or `N→N+1` projection pre-state. At the same sampled promotion time, the spawning Grant must still authorize the accepted adapter target and a continuation's accepted exact-prior session-management Grant must still be live, exact-scope, and bound to the same verified subject/endpoint. Revocation or expiry suppresses promotion; the owner never substitutes another Grant id into accepted provenance.
