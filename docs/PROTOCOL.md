...
A successful spawn `Result` is durable completion evidence, not by itself the public terminal transition. The core's fail-closed completion owner waits for the correlated `SessionRegistered` or `SessionGenerationBumped` fact, then holds the shared decision gate while it records the verified `CommandCompleted/spawn_completion` audit, the audit-linked descendant grant, and finally `CommandState = completed`. Startup replays and repairs any committed prefix before service projections or listeners open; a historical completed transition may repair a missing audit/grant but never receives a duplicate terminal transition. Thus readers using the core decision gate observe either the pre-completion prefix or the final authority-bearing completion, not an audit/grant-only public completion.
