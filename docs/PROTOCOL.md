...
| observability / diagnostics | adapter diagnostics as payload: adapter-process durable diagnostics log + adapter-reported diagnostics recorded by the core and surfaced in the cockpit's existing views | C | `epic-observability-dogfooding`; SPEC post-v0.1.0 observability scope; PROTOCOL Payload |
