...
The production core composes one fail-closed spawn-completion log consumer. It rebuilds the authority-domain prefix under the same `CoreDecisionGate` used by control and adapter decisions before service projections are constructed or listeners bind, then runs continuous catch-up as a peer of both serving futures. For a managed fresh or continuation claim, a successful Result remains deferred evidence and never terminalizes through generic Observation ingestion. The consumer requires the exact accepted claim and compound provenance, delivered/running lifecycle before success, one staged successor with its external-runtime reservation, exact `∅→1` or `N→N+1` pre-state, and promotion-time live spawning authority plus live exact-prior replacement authority for continuation. It then asks the storage port to commit one `SpawnPromotionCommitted` source and its immediately linked completion audit in one transaction. The aggregate projection stages and publishes that source in authority → session → claim → command order, so descendant authority exists before N+1 becomes current or the command becomes completed. A revoked or expired accepted Grant suppresses promotion and leaves the candidate staged for explicit reconciliation. The legacy audit → descendant Grant → terminal tail remains only as one-way repair for pre-managed histories; evidence-only, audit-only, audit+grant, and completed prefixes normalize without duplicate grants or terminal transitions. Qualifying durable deferred success still suppresses unsafe adapter redelivery.
