...
Snapshots repair missed streams and reconnect gaps when the adapter or core can provide an authoritative snapshot. Adapters with partial or no snapshot capability degrade as defined in `docs/PROTOCOL.md`. A resource adapter may claim only the tier supported by the complete external view it can actually reconstruct. Every core-materialized session/resource snapshot carries the authority domain's persisted nonzero storage-continuity epoch. A stored session checkpoint is usable only when its storage-row and embedded authority-domain/epoch/LSN anchors match exactly; freshness is then decided separately by LSN. `LoadSnapshot` explicitly selects and echoes the session or resource view so opaque bytes cannot be decoded as the wrong projection. Resource snapshots materialize on demand from the durable resource projection; the existing checkpoint namespace remains session-only until a future typed-checkpoint migration is justified.
...
- **Crash recovery**: on ordinary restart the core reloads the same durable continuity epoch and replays the log (or loads an exact domain/epoch/LSN-compatible checkpoint then replays the tail) to reconstruct in-memory state up to the last committed log sequence number. Accepted Operations are restored; no accepted Operation disappears silently. Incompatible derived checkpoints are discarded in favor of log replay/current materialization.
