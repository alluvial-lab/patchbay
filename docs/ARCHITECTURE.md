...
v0.1.0 commits one explicit adapter-scoped spawn boundary: `OperationKind = spawn` addresses one attached adapter before a target session exists, and the operation-aware resolver rejects runtime-session, operational-resource, fleet-supervisor, and authority-domain spawn targets before durable acceptance. Spawn variants (worktree, same-dir, session, process, cloud environment) remain payload `target_spec.shape` values from the reserved open shape registry, not per-variant OperationKinds. Intentional restart is another spawn Operation carrying an exact continuation prior; there is no restart OperationKind. Core orchestration owns the claim/fence, typed phase order, exact prior-N unavailable/unknown precondition, staged successor, and atomic promotion, while adapters own termination/replacement/reconciliation mechanics. Fleet-default adapter selection is a reserved rework seam, and broadcasting a non-idempotent spawn is explicitly excluded. See `docs/PROTOCOL.md` for target and authority semantics and `docs/SECURITY.md` for descendant grants and revocation.
...
The capability manifest declares a generated target category (`runtime_session` or `operational_resource`) and, for resources, exact open `ResourceKind` declarations with per-kind snapshot tier and payload/domain-projection schema bindings. The reserved `knowledge_bundle` category is wire-present for an OKF-v0.2 candidate contract but is rejected at registration until promoted. For managed spawn it may also declare bounded configured targets that bind a logical-target id and one supported shape to fresh and optional continuation opaque adapter-payload templates; canonical diagnostics carry these templates so surfaces do not decode adapter schemas. It also carries one complete versioned `AdapterAssuranceManifest`: V1 declares deduplication strength, continuation-proof support, cursor support, generation fencing, reconciliation strength, and the action for an unproven outcome. Fresh attachment requires every V1 dimension explicitly; durable pre-assurance registrations normalize only during replay to a complete conservative V1 view. An adapter may additionally carry one bounded generated adapter-specific profile through a generic opaque envelope. The core validates only envelope framing, schema-reference/content-type identity, and size; it never branches on Pi or another adapter's profile fields. Canonical diagnostics carry the generated assurance manifest once and only the safe profile presence/schema-version descriptor, omitting replay-only declarations, attachment descriptors, and opaque profile bytes.

Core registration validates and stores this projection once; exact `(adapter_id, resource_kind)` lookup is the admission boundary consumed by resource report ingress. Assurance is static declared capability, not installed/reachable state: attachment, a live delivery stream, snapshot availability, or session replacement cannot turn an uncertain dimension true. Every capability declaration remains advisory and never replaces grants, suppresses otherwise authorized delivery, terminalizes an Operation, or overrides adapter-authoritative delivery outcomes.
...
Snapshots repair missed streams and reconnect gaps when the adapter or core can provide an authoritative snapshot. Adapters with partial or no snapshot capability degrade as defined in `docs/PROTOCOL.md`. A resource adapter may claim only the tier supported by the complete external view it can actually reconstruct. Every core-materialized session/resource snapshot carries the authority domain's persisted nonzero storage-continuity epoch. Stored session checkpoints use a private typed, versioned envelope and are usable only when the envelope kind/version, storage-row anchor, embedded authority-domain/epoch/LSN anchors, explicit managed-lineage provenance, and complete session-projection semantics validate. Marked managed tombstones require exact session/logical-target symmetry; unmarked logical-target tombstones are disposable rather than inferred from ambiguous checkpoint shape. Legacy undiscriminated bytes and resource payloads are likewise disposable misses rather than session authority. The production writer best-effort refreshes one latest session checkpoint by observed-event gap; session recovery folds its validated tail and retries from LSN 0 if that tail disagrees with the derived seed. Every sibling projection still rebuilds its owned state from the full log; diagnostics only reuses the accepted seed for its embedded session view. `LoadSnapshot` explicitly selects and echoes the public session or resource view. Resource snapshots materialize on demand from the durable resource projection; typed composite/per-projection checkpoint namespaces remain reserved.
...
- **TypeScript Pi adapter** — a runtime-adapter process that supervises one `pi --mode rpc` subprocess per managed runtime generation, translates delivered Operations into Pi actions, and reports source-authenticated session state, results, and transcript Observations to the Rust core. Its per-runtime action gate is the sole managed stdin owner. A typed resource reload is an in-process, same-generation action admitted only while idle and settled on a materialized session; success waits for persisted request/completion markers, a greater challenged extension epoch, subscription rebind, and authoritative cursor reconciliation. Pi package aliases, arbitrary imported dependency graphs, compiled `/dist`, native modules, and executable changes remain process-replacement-only.
...
- **Log + snapshots**: the durable event log is the source of truth; snapshots are derived checkpoints used to reduce recovery replay cost. No derived checkpoint, replica, or projection may become an independent ordering or authority source. The store persists one nonzero core-assigned continuity epoch per authority domain; all materialized snapshots carry it.
- **Crash recovery**: on ordinary restart the core reloads the same durable continuity epoch and validates the full durable prefix. The complete session projection may load its typed/versioned exact domain/epoch/LSN- and semantic-compatible latest checkpoint and replay only the tail; wrong type/version/domain/epoch/LSN or payload returns no session seed and replays sessions from LSN 0. Authority, commands/inboxes, Elicitations, resources, diagnostics, security/operator, adapter, and operator-session projections still replay the full log, so v0.1.0 makes no whole-core bounded-recovery claim. Accepted Operations are restored; no accepted Operation disappears silently.
...
The production core composes one fail-closed spawn-completion log consumer. It rebuilds the authority-domain prefix under the same `CoreDecisionGate` used by control and adapter decisions before service projections are constructed or listeners bind, then runs continuous catch-up as a peer of both serving futures. For a managed fresh or continuation claim, a successful Result remains deferred evidence and never terminalizes through generic Observation ingestion. The generic restart path is accept/fence → deliver → durable exact-claim quiesce/prior-terminated/launch evidence → adapter-owned terminate/replace/reconcile → identity/handshake → exact successor staging → later success evidence → sole-driver promotion. The completion driver reconstructs this order from the durable LSN chain rather than caller-local sequence flags. The consumer requires the exact accepted claim and compound provenance, delivered/running lifecycle before success, ordered exact-runtime phase evidence (or later handshake/success after poison), one staged successor with its external-runtime reservation, exact `∅→1` or `N→N+1` pre-state, and promotion-time live spawning authority plus live exact-prior replacement authority for continuation. Continuation N must still be exact-current but offline/stale/failed with unknown activity before staging or promotion. It then asks the storage port to commit one `SpawnPromotionCommitted` source and its immediately linked completion audit in one transaction. The aggregate projection stages and publishes that source in authority → session → claim → command order, so descendant authority exists before N+1 becomes current or the command becomes completed. A revoked or expired accepted Grant suppresses promotion and leaves the candidate staged for explicit reconciliation. The legacy audit → descendant Grant → terminal tail remains only as one-way repair for pre-managed histories; evidence-only, audit-only, audit+grant, and completed prefixes normalize without duplicate grants or terminal transitions. Qualifying durable deferred success still suppresses unsafe adapter redelivery.
