...
| `/reload` | reserved / profile-contract-only until verified | The generated profile narrows the future action to the extension entrypoint plus Pi-enumerated skills/prompts/themes/context files on an idle materialized session. It does not advertise execution support. Arbitrary extension dependency graphs, Pi/runtime package `/dist`, native dependencies, the executable, and unknown scope require process replacement. |
