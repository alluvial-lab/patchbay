...
This is the core of the checklist. It maps each committed v0.1.0 `OperationKind` (the registry is authoritative in `docs/PROTOCOL.md`, `### OperationKind registry`) to the Pi wire action(s) that satisfy it, and records the capability-manifest declarations the Pi adapter makes. Per-Operation delivery mapping and manifest-field declarations are kept in separate columns: manifest fields come from the manifest shape in `docs/PROTOCOL.md` (Adapter capabilities) and correspond to the generated `AdapterCapability` fields in `contracts/rust/src/gen/patchbay/patchbay.rs` — `supported_operation_kinds`, `supported_target_spec_shapes`, `streaming_support`, `session_snapshot_support`, `cancellation_support`, `session_replacement_support`, complete `assurance.v1`, the optional opaque `adapter_profile`, `attachment_method`, `known_failure_modes`, `target_categories`, and `resource_capabilities`. The names below use the manifest dimensions as prose shorthand (e.g. `streaming` for the `streaming_support` field); a delivery outcome such as `unsupported_command` is an adapter-reported delivery result, not a manifest field.

| `OperationKind` | Pi wire action(s) | Manifest declaration (actual fields) | v0.1.0 disposition |
|---|---|---|---|
| `attach` | `session_sync` | `supported_operation_kinds` includes `attach`; `target_categories=[runtime_session]`; `streaming=true`; `session_snapshot=partial`; `cancellation=true`; `session_replacement=true` under the activated managed lifecycle | committed v0.1.0 |
| `instruct` | `user_message` | `supported_operation_kinds` includes `instruct` | committed v0.1.0 |
| `cancel` / `interrupt` | `cancel` | `supported_operation_kinds` includes both kinds and `cancellation=true`; Pi maps both to its correlated abort/cancel boundary | committed v0.1.0 |
| `approval-response` | `approve_tool` (approval Elicitation opened via the `tool_call` hook) | `supported_operation_kinds` excludes `approval-response` in the v0.1.0 Pi adapter manifest until the approval-Elicitation round-trip is implemented; the `beforeToolCall` hook is operational with audited auto-proceed/block. Delivery of an `approval-response` Operation returns `unsupported_command` until then. | committed v0.1.0 kind; Pi-adapter delivery support for the full Elicitation round-trip is a reserved adapter-level seam (the hook exists; the round-trip is additive) |
| `query` | `session_sync` / `list_models` / `ping` | `supported_operation_kinds` includes `query` | committed v0.1.0 |
| `reconfigure` | `model_set` / `thinking_set`; typed idle-only `ctx.reload()` for the loaded extension entrypoint and Pi-enumerated skills/prompts/themes/context files | `supported_operation_kinds` includes `reconfigure`; reload rejects before effect unless the managed RPC runtime is idle, settled, current, and backed by a strictly materialized session | committed v0.1.0 |
| `session-management` | `session_new` / `session_compact` | the kind is supported and `session_replacement=true`; managed exact continuation supplies the replacement proof, while `session_new` reports its observed generation bump and `session_compact` does not | committed v0.1.0 |
| `spawn` | supervised `pi --mode rpc` fresh launch or exact continuation | `supported_operation_kinds` includes `spawn`; `supported_target_spec_shapes=[pi-rpc]`; delivery requires a generated `PiSpawnTargetSpec` and the exact core claim/fence/provenance | committed kind; current Pi adapter support is activated by the managed-lifecycle conformance gate |
| `elicitation-response` | no distinct Pi non-approval question wire type; the `tool_call` approval gate is the closest | `supported_operation_kinds` excludes `elicitation-response` in the v0.1.0 Pi adapter manifest; non-approval `question` Elicitations unsupported by the Pi adapter at delivery (`unsupported_command`) until a Pi question surface is promoted | committed core kind + committed `question` contract; Pi-adapter support for non-approval question Elicitations is a reserved adapter-level seam |
...
**Target/snapshot, profile, and assurance declaration:** the Pi adapter declares only `target_categories = [runtime_session]`, `session_snapshot_support = partial`, and no resource capabilities. It carries one Protobuf `patchbay.PiRuntimeProfile.v1` inside the generic opaque profile envelope. That generated Pi-local profile names the JSONL RPC subprocess contract, partial live-event caveats, deferred session materialization, persisted-entry/exact-set cursor design, challenged control-extension proof, idle/materialized reload boundary, enumerated extension/skill/prompt/theme/context-file resources, adapter-resolved cwd/project trust/resource roots, and process-replacement-only exclusions. The core validates only envelope framing/size and does not decode these Pi values; diagnostics expose only profile presence/schema/content type.

The complete generic assurance V1 is evidence-backed and activated: deduplication is `at-Patchbay-boundary`; continuation proof, cursor, and generation fence are `true`; reconciliation is `bounded`; unproven external outcome is `manual-required`; and session replacement is `true`. `spawn` and managed target shape `pi-rpc` are present. Every configured managed target is also declared through the generic managed-spawn target registry with its logical-target id and distinct fresh/`require_resume` opaque payload templates. Those templates carry only the bounded non-path `project_context_ref` and generated continuation mode; cockpit and CLI pass them through without Pi-specific decoding. The bounded reconciliation claim is deliberately narrower than the authoritative exact-set transcript mechanism: persisted transcript membership can be replaced authoritatively inside its verified continuity scope, but the adapter cannot reconstruct every externally effecting Operation outcome. A launch left at `launch_attempted / may_exist` or a post-effect reload ambiguity therefore remains `execution_outcome_unknown` with manual reconciliation required. The profile schema itself activates none of these claims; the named managed-lifecycle evidence set and exact conformance version do. `session_sync`/`session_history`, attachment, reachability, or a similarly named profile field is not substitute evidence. Pi persisted entries projected as transcript events provide exact current membership in the cursor scope, not arbitrary Operation-outcome history. The core reconciles reconnects against the partial snapshot tier per the degraded-behavior rules in `docs/PROTOCOL.md`; it never fabricates a snapshot from cached state or claims operational resources.

The spawn recovery journal retains only unresolved per-claim effect/poison evidence and the full staged publication needed between promotion and publication commit. An active record admits at most 64 MiB—aligned with the strict session-file boundary and above the 16 MiB cursor-store record boundary. After promotion plus publication/cursor commit, the full projection compacts to a small duplicate-replay receipt retained for at most 24 hours and at most 128 completed claims; startup prunes expired/excess receipts and does not scan them as active recovery work. A safely abandoned pre-launch claim is deleted immediately. Ambiguous or poisoned claims are never evicted by completed-history retention.
...
| `/reload` | committed / bounded reconfigure mechanism | A typed `PiReconfigureRequest` enters the per-runtime exclusive action gate. Streaming, compaction, queued work, an unsettled retry/activity epoch, another delivery/direct RPC, stale runtime evidence, and memory-only/invalid sessions reject before the request marker or reload call. Success requires a physically materialized exact request, a matching materialized completion from a greater extension epoch, a new challenged handshake, process-local subscription rebind, and authoritative cursor reconciliation. Runtime generation, process identity, and logical target do not change. Only the loaded extension entrypoint and Pi-enumerated skills/prompts/themes/context files are reloadable; arbitrary extension dependency graphs, Pi/runtime package aliases or `/dist`, native dependencies, the executable, and unknown scope return process-replacement-required/unsupported rather than claiming refresh. |
