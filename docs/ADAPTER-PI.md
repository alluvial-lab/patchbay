...
**Target/snapshot and assurance declaration:** the Pi adapter declares only `target_categories = [runtime_session]`, `session_snapshot_support = partial`, and no resource capabilities. Its complete assurance V1 is evidence-backed and conservative: deduplication is `at-Patchbay-boundary`; continuation proof, cursor, and generation fence are `false`; reconciliation is `none`; and an unproven external outcome is `manual-required`. Session replacement, `session_sync`/`session_history`, attachment, and reachability prove none of the false/none dimensions. Pi persisted entries projected as transcript events provide recent/current state, not arbitrary historical reconstruction. The core reconciles reconnects against the partial snapshot tier per the degraded-behavior rules in `docs/PROTOCOL.md`; it never fabricates a snapshot from cached state or claims operational resources.
