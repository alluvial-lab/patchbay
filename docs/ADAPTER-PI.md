...
**Target/snapshot declaration:** the Pi adapter declares only `target_categories = [runtime_session]`, `session_snapshot_support = partial`, and no resource capabilities. Pi persisted entries projected as transcript events via `session_sync` → `session_history` provide recent/current state, not arbitrary historical reconstruction. The core reconciles reconnects against this tier per the degraded-behavior rules in `docs/PROTOCOL.md` (Adapter session and resource snapshot capability tiers); it never fabricates a snapshot from cached state or claims operational resources.
