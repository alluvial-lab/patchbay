...
|---|---|---|---|
| `attach` | `session_sync` | `supported_operation_kinds` includes `attach`; `target_categories=[runtime_session]`; `streaming=true`; `session_snapshot=partial`; `cancellation=true`; `session_replacement=false` until managed-lifecycle conformance activates the conditional claim | committed v0.1.0 |
...
| `session-management` | `session_new` / `session_compact` | the kind remains supported, but the static `session_replacement` guarantee is conservatively `false` until exact managed continuation proof is activated; `session_new` still reports its observed generation bump and `session_compact` does not | committed v0.1.0 |
...
The complete generic assurance V1 remains evidence-backed and conservative: deduplication is `at-Patchbay-boundary`; continuation proof, cursor, and generation fence are `false`; reconciliation is `none`; unproven external outcome is `manual-required`; and session replacement is `false`. `spawn` and managed target shapes remain absent. The profile schema itself activates none of those claims: the final lifecycle conformance checkpoint must name the exact conformance implementation before any stronger value is emitted. `session_sync`/`session_history`, attachment, reachability, or a similarly named profile field proves none of the false/none dimensions. Pi persisted entries projected as transcript events provide recent/current state, not arbitrary historical reconstruction. The core reconciles reconnects against the partial snapshot tier per the degraded-behavior rules in `docs/PROTOCOL.md`; it never fabricates a snapshot from cached state or claims operational resources.
