...
`SpawnCreatesDescendantGrant` now has mutation-sensitive implementation evidence over the production fold and composition seam: successful spawn results defer generic terminalization and suppress delivery reconstruction across reconnect/restart; exact parent-grant/verified-sender acceptance plus valid delivered/running LSN order exclude accepted-state and preseed authority; registration and generation-bump correlation converge only when the session target is contained by the accepted spawn scope; revocation command effects participate in the terminal winner; and descendant ingress rejects a self-consistent forged source/audit/grant chain without the full durable context. The driver uses shared gap-free replay validation, repairs evidence-only, audit-only, audit+grant, and complete crash prefixes, validates historical completed repair through the same lifecycle fold, preserves exact audit linkage across replay, and keeps staged completion audit output from claiming stderr completion before the final transition. Adapter-scoped real delivery/result/report/restart evidence authorizes a subsequent real existing-session `Submit` through the generated descendant grant and preserves independent parent/child revocation; current ordinary `Submit` target resolution still rejects adapter/fleet spawn targets, so the accepted spawn handoff in that path is seeded rather than misrepresented as an RPC-accepted fleet spawn. This evidence does not promote `authority.qnt`, does not promote the reserved `spawn-descendant-grant` vector, and is not described as checked-model or checked-normative assurance.
