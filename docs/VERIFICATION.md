...
- Crash recovery: no accepted command disappears silently after an ungraceful restart; idempotent log replay; session-only checkpoint tail recovery under a qualified healthy scheduling target, without a whole-core recovery bound.
...
Current stated-normative properties with no executable formula:
...
`FleetAuthorityForSpawn` remains in the draft model vocabulary as a **reserved future property**, because v0.1.0 resolves only an explicitly selected adapter target and deliberately has no fleet-default selector or broadcast path.

The first three general properties retain their documented refinements to `NoOperationWithoutGrant`, the operator-session issuer shape, and `GrantAuthorityIsOperationKinds`; those refinements do not promote the draft obligations. `SpawnCreatesDescendantGrant` requires a future model using the canonical descendant-grant OperationKind set and action-created grant state. The current adapter-scoped acceptance boundary and `ElicitationResponderAuthority` require submitted actor/endpoint/target evidence that the accepting action cannot rewrite. The reserved `FleetAuthorityForSpawn` property additionally requires a separately promoted deterministic fleet-selector model. `SpawnRevocationDoesNotCascade` requires a pre/post-state oracle that detects deletion of an existing descendant during parent-grant revocation.
...
Classification: the seven current property ids above remain **stated-normative until promoted** with genuine, mutation-survivable formulas. `FleetAuthorityForSpawn` remains draft/reserved until a future fleet-selector scope act promotes it.

`SpawnCreatesDescendantGrant` now has mutation-sensitive implementation evidence over the production fold and composition seam: a real `Submit` resolves and accepts one explicitly addressed attached adapter, while runtime-session/resource spawn targets reject before durable acceptance and fleet-default selection remains reserved; successful spawn Results defer generic terminalization and suppress delivery reconstruction across reconnect/restart; accepted-state, Result-before-delivery, missing staged successor, conflicting Result, wrong claim/provenance, non-exact generation, and reverse-owner conflicts cannot promote. Result-first and report-first orders converge on one promotion after both facts exist. The completion driver consults the durable authority projection at the promotion's sampled time, and mutations that ignore expired/revoked spawning or exact-prior replacement authority fail. The dedicated SQLite append stamps and commits one `SpawnPromotionCommitted` source plus its immediate completion audit in one transaction; crash-before leaves no promotion, lost-ack-after replays the complete source/audit pair, and aggregate publication folds authority → session → claim → command so no public N+1 or completed state precedes descendant authority. The legacy audit → grant → terminal tail is command-scoped to pre-managed histories and normalizes evidence-only, audit-only, audit+grant, and completed prefixes without duplicate grant/audit/terminal writes; a fresh or continuation `SpawnClaim` cannot enter that reactor. One exact-correlation qualifier makes identical duplicate command references eligible in observation ingestion, `CommandIndex`, the completion owner, and audit-source validation, with restart evidence proving a no-redelivery checkpoint cannot remain completion-ineligible. Real Submit → attached-adapter delivery → success + staged report → restart authorizes a subsequent real existing-session `Submit` through the generated descendant Grant and preserves independent parent/child revocation. Registration-boundary evidence additionally rejects generic authenticated registration-schema Event ingress before append, rejects every named canonical-envelope identity mutation during replay, and proves an adapter-A/embedded-adapter-B attempt leaves A's generation unchanged, B absent and not spawn-resolvable, while the valid `Attach` registration remains spawn-resolvable after replay. The shared complete-prefix validator still owns exact gap-free LSN validation. This evidence does not promote `authority.qnt`, does not promote the reserved `spawn-descendant-grant` vector, and is not described as checked-model or checked-normative assurance.

Restart-as-continuation adds implementation evidence, not a newly promoted formal property. Rust phase-table and integration tests require the exact prior to stay current but offline/stale/failed with unknown activity and require the durable `delivered → quiescing_prior → prior_terminated → launch_attempted → external_identity_known → handshake_reconciling → staged → success_evidence_reported` continuation chain. Focused probes reject missing quiesce/old-runtime evidence, handshake-before-identity, stage-before-handshake, and phase evidence before delivery; later handshake/success remains required after poison. Existing tests also kill early-N+1-publication, preserved-prior-availability, claim-release, and automatic-relaunch mutants. Core/server recovery tests exercise claim/fence reconstruction, stream-loss stale/unknown degradation, staged-only invisibility, promotion atomicity, and replay. The generated `ContinuationContextStatus` is adapter-filled on the exact continuation report, copied into staged/promoted evidence, and folded by shared operator-domain, CLI, and web consumers as `resumed`/`new_context`/`unknown` without claiming process-state restoration. These are implementation/conformance tests; the parent spawn properties remain stated-normative at their existing classification.
...
- A snapshot from a different authority domain or durable core-generation epoch is rejected outright. Stored session checkpoints first require the supported typed/versioned session envelope, then exact equality between the current authority domain/epoch, the embedded authority domain/nonzero epoch, and the positive storage-row/embedded snapshot LSN anchors; legacy undiscriminated bytes, resource payloads, unsupported versions, and malformed payloads are incompatible before the separate LSN freshness rule.
...
The implementation persists `CoreGeneration` per authority domain, carries it in session/resource materialization, and accepts a format-3 complete session checkpoint after file-backed restart only when typed framing, domain/epoch/row-and-embedded LSN, live-record/view-revision, tombstone, source-cursor, explicit managed-lineage provenance, and lockdown semantics validate. Marked managed tombstones require exact session/logical-target symmetry; unmarked session-only legacy history remains compatible, while an unmarked logical-target tombstone is disposable and replays from LSN 0. Format-2 payloads decode as unmarked legacy checkpoints under that rule. A seeded `SessionRegistry` rejects compacted covered-prefix re-feed and applies only the validated tail; legacy/wrong-type/version/domain/epoch/LSN/payload checkpoints, or a semantically inconsistent checkpoint/tail pair, replay sessions from LSN 0 and force prompt checkpoint replacement. The best-effort event-gap writer atomically retains one latest session row and retries observable write failures without changing the log. The promoted `snapshot-reconciliation` executable example stores an otherwise compatible session checkpoint at its declared stale LSN, proves the RPC returns the newer current view, and constrains field carriage for both view kinds. This is implementation evidence only: `SnapshotCrossDomainRejected` remains stated-normative, and neither this vector extension nor the draft model is a checked-model or checked-normative promotion.
...
- Session checkpointing reduces the `SessionRegistry` recovery fold to a validated post-anchor tail under healthy writer scheduling without becoming an alternate ordering authority. The writer targets each 256 observed events with one-second polling, but failure/starvation/overload may exceed that gap. Every load-bearing non-session projection still full-replays, so whole-core recovery remains unbounded by this checkpoint. Ordinary restart preserves the authority domain's durable `CoreGeneration`; incompatible type/version/domain/epoch/LSN/payload/semantic state produces no session seed and replays sessions from LSN 0.

Normative whole-core model variables should include at least `CommittedPrefixLSN` (the last durably committed log prefix), `CheckpointSnapshotLSN` (the latest snapshot loaded before tail replay), `RecoveredCommandState` (the command-id to `CommandState` map reconstructed from the log), `RecoveredInbox` (delivery/inbox queue state reconstructed from the log), `RecoveredSessionView` (session connectivity/activity axes reconstructed from the log), and `RecoveryPhase` (initial load vs tail-replay vs live). `Crash` and `Restart` are the transition triggers. The current `snapshot_recovery.qnt` remains a draft command-oriented abstract whole-core model and is not evidence for the production session-only writer or its scheduling policy.
...
### Adapter target-category and resource projection evidence (implementation-checked)

The post-v0.1 capability-manifest extension is implementation-checked. Generated Rust/TypeScript contract builds cover the closed `AdapterTargetCategory` registry, tag-preserving session snapshot rename, per-resource snapshot declarations, and schema descriptor/projection shapes. Rust boundary and replay tests exercise exact category/declaration relationships, duplicate/unknown/reserved fail-closed behavior, replay-only legacy session normalization, exact `(adapter_id, resource_kind)` lookup, and exact payload/projection descriptor matching. Server/Pi/diagnostics/CLI tests cover fresh attach, no durable adapter-registration append on reserved OKF-v0.2 rejection, honest Pi session-only declaration, redacted durable projection, and session-versus-resource tier presentation.
...
The diagnostics projection retains the canonical validated capability alongside each replayed registration and emits its generated V1 assurance once in `AdapterCapabilitySummary`; unknown replay-only deduplication normalizes to `none`, and descriptor/profile bytes are structurally absent. Server tests preserve exact assurance through fresh attach, durable replay, same-generation redeclaration, and newer-generation replacement while invalid declarations retain the existing no-append/no-token behavior. Pi and token-commune constructor tests require every V1 field explicitly: before the managed-lifecycle activation Pi advertised only Patchbay-boundary deduplication and manual-required ambiguity, while token-commune's partial/latest-50 view remains `none`/false rather than an authoritative cursor or outcome claim. CLI tests fail closed on missing/incomplete V1 and render generated dimension names plus the closed `unknown`/`manual-required` qualifier. Web tests derive retry safety from the canonical failure term plus V1 deduplication and reject capability-only retry decisions. With the Pi managed-lifecycle activation (`pi-managed-lifecycle.v2`), Pi's V1 declaration additionally advertises the evidence-backed continuation proof, cursor support (Pi-session-scoped authoritative replacement), and generation-fence support proven by the lifecycle conformance checkpoints, and declares bounded (`BOUNDED`) reconciliation strength: authoritative for transcript replacement, manual-required for unresolved launch/reload outcomes; token-commune's partial/latest-50 view remains `none`/false rather than an authoritative cursor or outcome claim.
...
`contracts/vectors/` registry. The umbrella checker runs sixteen exact package
...
nine exact scenario checks and kill 37 declared mutation witnesses before the
...
| Property id | Executable vector | Product seam and independent oracle | Declared killed mutation ids (validated from vectors) | Assurance tier |
|---|---|---|---|---|
| `TokenCommunePartialSnapshotHonesty` | `token-commune-partial-snapshot-honesty` | real projector over raw endpoint availability; literal two-view/PARTIAL/listed-only/no-aggregate oracle | `partial-to-authoritative`<br>`drop-declared-view`<br>`reuse-prior-successful-slice`<br>`missing-telemetry-to-zero`<br>`synthesize-capacity-aggregate` | promoted vector + implementation-checked; not model-checked |
| `TokenCommuneBoundedReconnectHonesty` | `token-commune-bounded-reconnect-honesty` | real latest-50 tracker/poller; independent set/sequence and report-order model | `replay-initial-history`<br>`advance-dedup-before-ack`<br>`suppress-no-anchor-gap`<br>`fabricate-missed-count`<br>`events-before-reconnect-report` | promoted vector + implementation-checked; not model-checked |
| `TokenCommuneDegradationHonesty` | `token-commune-degradation-honesty` | real failed poll plus real-core disconnect/reconnect snapshots; independent confidence truth table | `skip-empty-partial-report`<br>`disconnect-remains-current` | promoted vector + implementation-checked; not model-checked |
| `TokenCommuneCurrentGenerationSourceAuthenticated` | `token-commune-current-generation-source-authenticated` | real `AdapterControlService` attachment/token/generation/ownership ingress; attempted-pre-state tuple oracle | `ignore-generation-equality`<br>`accept-prior-attachment-token`<br>`compare-local-id-only` | promoted vector + implementation-checked; not model-checked |
| `TokenCommuneGatewayMemberKeyRedacted` | `token-commune-gateway-key-redaction` | real 0600 credential + HTTP client + reports/Observations/diagnostics/query/snapshot/subscription/SQLite scans; independent multi-encoding byte oracle | `allow-credential-reflection`<br>`remove-local-secret-replacement`<br>`leak-credential-path-in-diagnostics`<br>`forward-arbitrary-error-text` | promoted vector + implementation-checked; not model-checked |
| `TokenCommuneAdapterFailureSafe` | `token-commune-unsupported-operation-terminalization` | real adapter process and real-core same-process/replacement recovery; literal durable lifecycle oracle | `filter-delivered-nonterminal-on-replacement`<br>`advance-later-delivery-first`<br>`terminalize-completed`<br>`use-adapter-unavailable`<br>`duplicate-delivery-or-terminal-transition` | promoted vector + implementation-checked; not model-checked |
| `TokenCommuneCockpitPresentationHonesty` | `token-commune-cockpit-presentation-honesty` | exact adapter projection fixture through local decoder/compositor/bounded resource-Observation projection/option-7 panel; literal summary/DOM oracle | `style-stale-as-live`<br>`drop-unknown-row`<br>`join-draw-by-provider-only`<br>`accept-forbidden-gpt-5-6-alias`<br>`display-cross-provider-model`<br>`drop-carried-capabilities`<br>`hide-current-wrapper-old-reading-age`<br>`label-missing-reading-current`<br>`drop-resource-events`<br>`expose-contributor-member-subkey`<br>`trust-adapter-verdict`<br>`load-dynamic-renderer`<br>`render-hostile-html-script` | promoted vector + implementation-checked; not model-checked |
...
- `spawn-fleet-authority`: reserved future selector/authority example; it is not a v0.1.0 path and must not be promoted until deterministic fleet-default selection (without unsafe broadcast) is designed.
...
| `TokenCommuneCockpitPresentationHonesty` | stated-normative | [token-commune-cockpit-presentation-honesty](../contracts/vectors/token-commune-cockpit-presentation-honesty.json) (promoted) | patchbay.Observation.kind<br>patchbay.Observation.payload<br>patchbay.Observation.target_scope<br>patchbay.Resource.freshness<br>patchbay.Resource.projection_payload<br>patchbay.ResourceFreshnessState.RESOURCE_FRESHNESS_STATE_STALE<br>patchbay.ResourceFreshnessState.RESOURCE_FRESHNESS_STATE_UNKNOWN<br>patchbay.ResourceViewRevision.completeness |
