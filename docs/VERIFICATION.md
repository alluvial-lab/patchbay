...
`SpawnCreatesDescendantGrant` now has mutation-sensitive implementation evidence over the production fold and composition seam: a real `Submit` resolves and accepts one explicitly addressed attached adapter, while runtime-session/resource spawn targets reject before durable acceptance and fleet-default selection remains reserved; successful spawn results defer generic terminalization and suppress delivery reconstruction across reconnect/restart; exact parent-grant/verified-sender acceptance plus valid delivered/running LSN order exclude accepted-state and preseed authority; registration and generation-bump correlation converge only when the session target is on the accepted adapter; revocation command effects participate in the terminal winner; and descendant ingress rejects a self-consistent forged source/audit/grant chain without the full durable context. One exact-correlation qualifier makes identical duplicate command references eligible in observation ingestion, `CommandIndex`, the completion tail, and audit-source validation, with restart evidence proving a no-redelivery checkpoint cannot remain completion-ineligible. The driver uses shared gap-free replay validation, repairs evidence-only, audit-only, audit+grant, and complete crash prefixes, validates historical completed repair through the same lifecycle fold, preserves exact audit linkage across replay, exposes only a bounded redacted non-terminal `spawn_completion_deferred` diagnostic before issuance, and keeps staged completion audit output from claiming stderr completion before the final transition. Real Submit → attached-adapter delivery → success + registration/generation evidence → restart authorizes a subsequent real existing-session `Submit` through the generated descendant grant and preserves independent parent/child revocation. Registration-boundary evidence additionally rejects generic authenticated registration-schema Event ingress before append, rejects every named canonical-envelope identity mutation during replay, and proves an adapter-A/embedded-adapter-B attempt leaves A's generation unchanged, B absent and not spawn-resolvable, while the valid `Attach` registration remains spawn-resolvable after replay. The shared complete-prefix validator still owns exact gap-free LSN validation. This evidence does not promote `authority.qnt`, does not promote the reserved `spawn-descendant-grant` vector, and is not described as checked-model or checked-normative assurance.
