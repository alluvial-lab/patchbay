...
Restart-as-continuation adds implementation evidence, not a newly promoted formal property. Rust phase-table and integration tests require the exact prior to stay current but offline/stale/failed with unknown activity, require ordered exact-runtime identity → handshake → stage → success evidence, require later handshake/success after poison, and kill early-N+1-publication, preserved-prior-availability, claim-release, and automatic-relaunch mutants. Core/server recovery tests exercise claim/fence reconstruction, stream-loss stale/unknown degradation, staged-only invisibility, promotion atomicity, and replay. Shared operator-domain, CLI, and web tests prove fresh/restart both encode as `OperationKind.SPAWN`, restart carries the exact prior logical/runtime generation, and context copy stays within `resumed`/`new_context`/`unknown` without claiming process-state restoration. These are implementation/conformance tests; the parent spawn properties remain stated-normative at their existing classification.
