...
| `TokenCommuneCockpitPresentationHonesty` | `token-commune-cockpit-presentation-honesty` | exact adapter projection fixture through local decoder/compositor/option-7 panel; literal summary/DOM oracle | `style-stale-as-live`<br>`drop-unknown-row`<br>`join-draw-by-provider-only`<br>`accept-forbidden-gpt-5-6-alias`<br>`expose-contributor-member-subkey`<br>`trust-adapter-verdict`<br>`load-dynamic-renderer`<br>`render-hostile-html-script` | promoted vector + implementation-checked; not model-checked |
