...
2. **Server-streaming** — `Subscribe(cursor) returns (stream SubscribeEvent)` delivers an async iterable of events to the Node client over HTTP/2. Four events arrived in LSN order (101-104), each as a distinct HTTP/2 DATA frame (a 5ms server-side delay confirmed real streaming rather than a fused batch). Connect-ES `createClient().subscribe()` returns an async iterable directly.
