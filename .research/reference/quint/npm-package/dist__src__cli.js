...
const runCmd = {
    ...
    //      })
    handler: (args) => {
        (0, cliCommands_1.load)(args).then(chainCmd(cliCommands_1.parse)).then(chainCmd(cliCommands_1.typecheck)).then(chainCmd(cliCommands_1.runSimulator)).then(cliCommands_1.outputResult);
    },
};
...
const verifyCmd = {
    ...
    handler: (args) => {
        if (args.verbosity === undefined) {
            args.verbosity = args.backend === 'tlc' ? 3 : verbosity_1.verbosity.defaultLevel;
        }
        (0, cliCommands_1.load)(args)
            .then(chainCmd(cliCommands_1.parse))
            .then(chainCmd(cliCommands_1.typecheck))
            .then(chainCmd(cliCommands_1.compile))
