...
fn issuer_headers(
    ...
    request
        .metadata_mut()
        .insert("x-patchbay-principal-id", principal_id.parse().unwrap());
    request.metadata_mut().insert(
        "x-patchbay-principal-secret",
        principal_secret.parse().unwrap(),
    );
    request
        .metadata_mut()
        .insert("x-patchbay-operator-id", ACTOR.parse().unwrap());
    request.metadata_mut().insert(
...
async fn entry_restart_and_credential_independent_admin_exit_recover_posture() {
    ...
    let exit = AdminServiceImpl::new(
        restarted.clone(),
        SetupSecret::new("unused".to_owned(), std::time::Duration::from_secs(60)),
    )
    ...
    let next = restarted
        ...
        .issue_operator_session(OperatorSessionBinding {
            ...
            endpoint_id: patchbay_contracts::patchbay::EndpointId {
                value: "cli-2".to_owned(),
            },
            device_id: patchbay_contracts::patchbay::DeviceId {
                value: "console-2".to_owned(),
            },
    ...
    let page = storage
        .query_audit(
            ...
            AuditPageSpec {
                kinds: vec![
                    AuditEventKind::LockdownEntered,
                    AuditEventKind::LockdownExited,
                ],
                actor_id: None,
                endpoint_id: None,
                command_id: None,
    ...
    assert_eq!(
        entered_audit.actor_id.as_ref().map(|id| id.value.as_str()),
        Some(ACTOR)
    );
    assert_eq!(
        entered_audit
    ...
    assert_eq!(
        entered_audit.device_id.as_ref().map(|id| id.value.as_str()),
        Some("console")
    );
    assert_eq!(entered_audit.reason_code, "suspected_endpoint_compromise");
    assert_eq!(
        entered_audit.target_scope.as_ref().map(|scope| scope.kind),
        Some(TargetScopeKind::AuthorityDomain as i32)
    );
    ...
    assert_eq!(
        exited_audit.target_scope.as_ref().map(|scope| scope.kind),
        Some(TargetScopeKind::AuthorityDomain as i32)
    );
}
